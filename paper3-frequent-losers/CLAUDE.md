...
- ✅ **D1 — horse race harmonized same-sample (script 36): RE-RUN 2026-05-25, PASS.** With the corrected FL14 = `tenders_count ≥ 14` cut: continuous AUC **0.939** [0.932, 0.946] dominates FL14 AUC **0.924** [0.921, 0.926]; DeLong **Z=−4.38, p=1.2e-05**; price coefficients same sign. (Pre-fix the 0.911/p=1.7e-5 was a `> 14` ≡ FL15 computation, fixed in script 36 lines 45 + 57 on 2026-05-22.) **Correction to the earlier prediction:** the gap shrinking 0.028→0.015 does NOT loosen significance — DeLong is a covariance-paired test, so the corrected p (1.2e-05) is in fact slightly *tighter* than the buggy 1.7e-05. Direction preserved either way. Stale FL15-bug macros (`\valHorseAUCBin`, `\valAUCFLBinSameSample` = 0.911; `\valDeLongZ` = −4.30; `\valDeLongP` = 2e-05) were corrected in `values.tex`/`paper_submission_clean.tex` (all were unused in prose, so no PDF changed); `output/gate_d1/d1_auc.csv` refreshed. Commit `d586567` on `v20`.
