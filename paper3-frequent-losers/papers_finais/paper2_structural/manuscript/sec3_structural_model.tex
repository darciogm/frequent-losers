...
model serves as a mechanism-organizing framework with three aims:
(i)~to formalize the economic logic behind the frequent-loser
phenomenon; (ii)~to specify a mixture likelihood that, given the FL
partition, separates FL-labeled bids from genuine bids; and
(iii)~to provide a coherent structural interpretation for the
observation that FL-labeled bids are less dispersed than genuine
bids---what we call the \emph{dispersion paradox}.
...
\begin{description}
  \item[A1 (Surplus structure).] $\pi(m, n) = b^*(m, n) - c_{j^*}$
    ...
    ($\partial b^* / \partial m \geq 0$). Three mechanisms support this
    assumption. \emph{First}, cover bids anchor the reference price:
    when $m$ additional bids cluster near $r$, the procurement
    agency's assessment of a ``competitive'' price shifts upward,
    allowing the cartel to sustain a higher $b^*$ without triggering
    review.\footnote{In BEC, the reference price $r$ is publicly posted
    and updated based on submitted bids; cover bids near $r$ validate
    the posted price and reduce scrutiny of the winning bid.}
    \emph{Second}, a higher total bidder count ($n + m$) creates the
    appearance of competition for oversight bodies, making $b^*$
    consistent with competitive expectations.
    \emph{Third}, if genuine bidders infer market tightness from the
    observed number of competitors, additional cover bidders may deter
    entry or induce upward bid shading, indirectly raising $b^*$.
    We do not model genuine-bidder entry endogenously; A1 is maintained
    as a reduced-form assumption, and the reduced-form price
    associations (Section~\ref{sec:reduced_form}) provide the empirical
    content.

  \item[A2 (Diminishing returns).] $\partial^2 \pi / \partial m^2 < 0$:
    the marginal return to cover bidding is decreasing. Note that A2
    restricts concavity in $m$ (holding $n$ fixed); it does not
    restrict the cross-partial $\partial^2 \pi / \partial m \, \partial
    n$, which Proposition~\ref{prop:market_selection} shows to be
    positive (strategic complementarity).
...
Regime~2 fits better empirically, it suggests that the preg\~ao
format---designed to enhance competition---may inadvertently
facilitate a more effective form of cover bidding.
...
between groups: if a fraction $\rho$ of FL-labeled bids are
actually genuine, $\hat{\epsilon}_{\text{noisy}} =
(1-\rho) \epsilon$. Our estimates of $\hat{\epsilon}$ and
...
The model generates five additional predictions, each tested on
independent data or moments not used in estimation (a sixth,
Prediction~\ref{pred:rationality} on cover-bidder characteristics,
was presented in Section~\ref{sec:cover_participation}):
...
\begin{prediction}[Regime-distinguishing dispersion pattern]
\label{pred:regime}
Under Regime~1, $\sigma_c > \sigma_g$. Under Regime~2,
$\sigma_c < \sigma_g$. BIC selects the better-fitting regime.
\end{prediction}
