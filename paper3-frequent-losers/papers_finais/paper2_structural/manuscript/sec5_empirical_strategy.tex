...
return to cover bidding, decreasing in $n$ by A2. Taking logs:
\begin{equation}
  \log m^*_{kt} = \log \pi'(n_{kt}) - \log(c_1 + \theta_k \phi_0)
  \label{eq:log_foc}
\end{equation}

We parameterize $\pi'(n) = \pi_0 \cdot n^{-\gamma}$ (diminishing
returns to cover bidding as genuine competition increases) and
$\theta_k = \bar{\theta} \cdot \text{PBUsize}_k^{\psi}$ (detection
probability increasing in oversight capacity). This yields a
...
$\gamma$ is identified from the cross-PBU covariation of $\bar{m}$
with $\bar{n}$ (holding PBU size fixed) and is the best-determined
parameter ($R^2 = 0.585$ from the log-linear first stage alone).
...
limiting the precision of $\phi_0$ while leaving $\gamma$ and $c_1$
well identified.
