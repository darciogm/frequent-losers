...
$+0.29$) as well as between PBUs ($+0.45$), which is inconsistent
with the association arising solely from cross-sectional selection.
...
practice, this gradient is small and only marginally significant
($-0.018$, $p = 0.024$), indicating that $\phi_0$ is weakly
identified. We cannot reject that enforcement has zero effect on
cover-bidder deployment. This is itself a substantive finding: cartel
cover-bidding intensity responds to competitive pressure, not to
oversight---though we acknowledge limited precision on $\phi_0 / c_1$.
