# ============================================================================
# 00_setup.R — Shared setup for Paper 3 v4 pipeline
...
# All v4 scripts source this file at the top.
# ============================================================================
...
for (pkg in c("bacondecomp", "did", "HonestDiD", "survival", "igraph", "ggraph", "tidygraph")) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    cat(sprintf("  Installing %s...\n", pkg))
    if (pkg == "HonestDiD") {
      if (!requireNamespace("remotes", quietly = TRUE))
        install.packages("remotes", repos = "https://cloud.r-project.org", quiet = TRUE)
      remotes::install_github("asheshrambachan/HonestDiD", quiet = TRUE)
    } else {
...
# ---- Path constants ----------------------------------------------------------
.v4_dir <- if (exists(".v4_dir")) .v4_dir else {
  normalizePath(file.path(dirname(sys.frame(1)$ofile %||% "."), ".."),
                mustWork = FALSE)
}

BASE_V1    <- normalizePath(file.path(.v4_dir, ".."), mustWork = FALSE)
DATA_V1    <- file.path(BASE_V1, "data", "processed")
DATA_V4    <- file.path(.v4_dir, "data", "processed")
OUT_TAB    <- file.path(.v4_dir, "output", "tables")
OUT_FIG    <- file.path(.v4_dir, "output", "figures")
OUT_LOG    <- file.path(.v4_dir, "output", "logs")

for (d in c(DATA_V4, OUT_TAB, OUT_FIG, OUT_LOG))
...
DATA_CACHE_V4_FULL    <- "/tmp/p3v4_prepared_full.rds"
MODELS_CACHE_V4       <- "/tmp/p3v4_models.rds"
NETWORK_CACHE_V4      <- "/tmp/p3v4_network.rds"
IV_CACHE_V4           <- "/tmp/p3v4_iv.rds"
IV_MODELS_CACHE_V4    <- "/tmp/p3v4_iv_models.rds"
BAJARI_CACHE_V4       <- "/tmp/p3v4_bajari_ye.rds"
MECHANISMS_CACHE_V4   <- "/tmp/p3v4_mechanisms.rds"
DID_CACHE_V4          <- "/tmp/p3v4_did.rds"
CADE_CACHE_V4         <- "/tmp/p3v4_cade.rds"
ROBUSTNESS_CACHE_V4   <- "/tmp/p3v4_robustness.rds"
WELFARE_CACHE_V4      <- "/tmp/p3v4_welfare.rds"
FL_ROBUST_CACHE_V4    <- "/tmp/p3v4_fl_robust.rds"
REGIME_CACHE_V4       <- "/tmp/p3v4_regime.rds"

# God Mode additions (v12 → v13)
STACKED_DID_CACHE_V4  <- "/tmp/p3v4_stacked_did.rds"
OSTER_CACHE_V4        <- "/tmp/p3v4_oster.rds"
DYADIC_CACHE_V4       <- "/tmp/p3v4_dyadic_perm.rds"
COX_CACHE_V4          <- "/tmp/p3v4_cox.rds"
...
#' Run 4 specifications for one DV (Equation 1 in manuscript)
run_losers_4 <- function(dv, data, treatment = "losers", price_only = FALSE) {
  dt <- copy(data)
  ...
  models[["general"]] <- feols(
    as.formula(paste0(dv, " ~ ", treatment, " + convite | item_f + year_f")),
    data = dt, cluster = ~item_f, fixef.rm = "none"
  )

  models[["general_pbu"]] <- feols(
    as.formula(paste0(dv, " ~ ", treatment, " + convite | item_f + year_f + pbu_f")),
    data = dt, cluster = ~item_f, fixef.rm = "none"
  )

  models[["pregao"]] <- feols(
    as.formula(paste0(dv, " ~ ", treatment, " | item_f + year_f + pbu_f")),
...
# ---- Generic 2-column regression table helper --------------------------------
write_2col_table <- function(models_list, caption, label, filename,
                              coef_var = "losers", coef_label = "FL presence",
                              col_labels = NULL, notes = NULL, d = 4) {
  n_cols <- length(models_list)
  if (is.null(col_labels)) col_labels <- paste0("(", seq_len(n_cols), ")")
  ...
  lines <- c(
    ...
    sprintf("\\begin{tabular}{l%s}", paste(rep("c", n_cols), collapse = "")),
    "\\toprule",
  ...
  # Coefficient row
  vals <- sapply(models_list, function(m) {
    if (coef_var %in% names(coef(m))) coef_cell(m, coef_var, d) else "---"
  })
  lines <- c(lines, sprintf("%s & %s \\\\", coef_label, paste(vals, collapse = " & ")))

  # SE row
  ses <- sapply(models_list, function(m) {
    if (coef_var %in% names(coef(m))) se_cell(m, coef_var, d) else ""
  })
  lines <- c(lines, sprintf(" & %s \\\\", paste(ses, collapse = " & ")))
  ...
  # N row
  obs <- sapply(models_list, function(m) pfmt_int(m$nobs))
  lines <- c(lines, sprintf("Observations & %s \\\\", paste(obs, collapse = " & ")))

  # R2 row
  r2 <- sapply(models_list, function(m) pfmt(fitstat(m, "r2")[[1]], 4))
  lines <- c(lines, sprintf("R-squared & %s \\\\", paste(r2, collapse = " & ")))
