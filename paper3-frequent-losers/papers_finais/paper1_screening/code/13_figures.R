...
# ============================================================================
# Figure 15: Density shift — FL vs non-FL bid spreads
# ============================================================================

cat("  Figure 15: Density shift (FL vs non-FL bids)...\n")

# Load bid-level data for density comparison
...
if (file.exists(bl_file)) {
  ...
  if (!is.na(price_col)) {
    ...
    bl <- bl[!is.na(bid_price) & bid_price > 0]
    bl[, is_fl := as.integer(firm_id %chin% fp[tenders_count > threshold, firm_id])]

    # Winning price per tender
    win_p <- bl[won == 1L, .(win_price = min(bid_price)), by = .(oc_code, item_code)]
    bl <- merge(bl, win_p, by = c("oc_code", "item_code"), all.x = TRUE)
    bl <- bl[!is.na(win_price) & win_price > 0]
    bl[, log_ratio := log(bid_price / win_price)]

    # Losing bids only (ratio > 0)
    bl_losers <- bl[won == 0L & log_ratio > 0 & log_ratio < 3]
    bl_losers[, group := fifelse(is_fl == 1L, "FL Bids", "Non-FL Bids")]

    # Compute median shift for annotation
    med_fl  <- median(bl_losers[is_fl == 1L, log_ratio], na.rm = TRUE)
    ...
    save_pub(p15, "fig_density_shift.pdf")
    rm(bl, bl_losers, win_p); gc(verbose = FALSE)
  }
} else {
  cat("    bid_level_full.parquet not found. Skipping density shift.\n")
}
