...
the IQR threshold may not have collusion-related participation, and
some collusion-related firms may escape the threshold. Under the plausible assumption that false positives (non-collusive
...
As a supporting diagnostic, we estimate a two-stage mixture model on
bid-level data (23.2 million non-FL losing bids and 189,381 FL losing
bids) and use BIC to select between the two cover-bidding regimes.
The full likelihood, identification argument, and estimation details
are in Appendix~\ref{app:structural_id}. This exercise is secondary
to the detection-performance analysis above; its role is to assess
whether FL-bid distributions are coherent with the framework's
coordinated-regime logic.
