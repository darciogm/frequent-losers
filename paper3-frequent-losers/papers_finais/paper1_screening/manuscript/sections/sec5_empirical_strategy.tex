...
(Section~\ref{sec:reduced_form}). We then check the screen's
detection performance against CADE cartel convictions and benchmark
it against bid-level screens
(Section~\ref{sec:detection_validation}). Finally, we test whether
the screen's behavior matches the cover-bidding model's
predictions, drawing on structural estimation
...
confirms that OLS understates the association; our primary range is
