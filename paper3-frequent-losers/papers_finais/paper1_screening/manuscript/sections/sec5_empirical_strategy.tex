...
conditional price association using OLS with high-dimensional fixed
effects, temporal cross-fitting, and matching
(Section~\ref{sec:reduced_form}). We then evaluate the screen's
detection performance against CADE cartel convictions and benchmark
it against bid-level screens
(Section~\ref{sec:detection_validation}). Finally, we report
diagnostics assessing whether the screen's behavior is coherent with
coordinated cover bidding, drawing on structural estimation
(Section~\ref{sec:structural_estimation}), Bajari--Ye
bid-coordination tests (Section~\ref{sec:bajari_ye}), and
network-split heterogeneity
(Section~\ref{sec:empirical_network}). These exercises play different
roles: the price results document an association, the CADE analysis
evaluates screening performance, and the remaining diagnostics assess
whether the observed pattern is coherent with the proposed
interpretation.
