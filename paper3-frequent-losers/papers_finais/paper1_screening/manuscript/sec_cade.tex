...
by high participation volume, which is the anomaly the screen is designed
to exploit. The permutation test (3.5$\times$, stratified by
participation quartile) indicates that the excess is real, not
mechanical. For enforcement purposes, this is the relevant target:
a triage rule that flags environments warranting closer scrutiny,
not a tool that adjudicates firm-level liability or substitutes for
...
full-sample 0.064 (Table~\ref{tab:excl_cade}). The conditional price
association is not driven by tenders CADE has already flagged. This is
the screen's practical value: it flags environments beyond the reach
of existing enforcement records, directing investigative resources
toward markets that would otherwise go unscrutinized.
