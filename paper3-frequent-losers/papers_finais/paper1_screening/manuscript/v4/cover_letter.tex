...
Applied to 4.5 million tender-items from S\~ao Paulo's electronic procurement platform (2009--2019), the screen achieves AUC~$= 0.94$ against competition-authority convictions. FL-flagged tenders show 3.6--7.7\% higher conditional prices across four estimation approaches. Five supporting diagnostics are coherent with coordinated cover bidding but do not claim mechanism identification; the paper's contribution is the screen itself and its external validation.
...
\item \textbf{Institutional design implications.} The data are coherent with the view that Brazil's minimum-bidder rule in sealed-bid procurement (convite) creates an incentive for cover bidding. Brazil's recent procurement reform (Lei 14.133/2021), which restructures bidding modalities and minimum-bidder rules, provides a natural laboratory for assessing whether the screen adapts to institutional change.
\end{enumerate}
