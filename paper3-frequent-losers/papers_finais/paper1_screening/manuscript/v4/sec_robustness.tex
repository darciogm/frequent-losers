...
We stress-test the baseline from five angles:
...
homogeneous subsamples, item$\times$year FE, matching); alternative
clustering; sensitivity to unobservables (Cinelli--Hazlett); and
staggered DiD. Table~\ref{tab:robustness_summary} collects the
point estimates alongside the identifying assumption each estimator
requires.
...
Stricter thresholds yield smaller coefficients (0.079 at
1.0$\times$, 0.064 baseline, 0.060 at 2.0$\times$, 0.050 at
3.0$\times$), likely because classification noise increases as the
...
\paragraph{Coefficient stability and cost-benefit.}
The \citet{oster2019unobservable} bound is degenerate
($\hat{\delta} = 261.6$) because PBU FE barely move $R^2$ (0.879 to
0.886); the \citet{cinelli2020making} framework ($RV = 17.5\%$) is
the appropriate sensitivity metric. Cost-benefit calculations for
screening deployment appear in Appendix~\ref{app:extensions}.
