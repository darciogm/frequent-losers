...
classification fills this gap: a data-driven, \emph{ex ante} partition
grounded in a participation anomaly that feeds directly into
Bajari--Ye tests (Section~\ref{sec:results_bajari}).
...
workflow: FL flags triage firms, bid-level analysis follows on
flagged tenders. In a direct comparison on the same ground truth, FL
achieves AUC~$= 0.94$ versus 0.79 for an Imhof-style CV proxy, and
the two screens are nearly orthogonal (correlation 0.06).%
...
caught. We model cover bidding \emph{ex ante}, distinguishing
two regimes with different bid distributions and, crucially, different
detection vulnerabilities. The coordinated regime defeats the
dispersion-based screens that dominate the literature, creating a
detection gap that the FL screen is designed to fill.

\paragraph{Procurement in developing economies.}
The bid-rigging literature concentrates on developed economies, yet
the stakes are highest where enforcement resources are scarce
\citep{sanchezgraells2019screening}. BEC's scale---4.5 million
tender-items, 40 million bids, 11 years, two auction formats---provides a demanding test bed, and the screen's minimal data
requirements make it portable to settings where bid-level forensics
are infeasible.
