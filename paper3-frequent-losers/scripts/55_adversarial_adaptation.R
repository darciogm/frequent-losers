...
cobidder labels) fixed. The construct's signal degrades under
adaptations that mechanically attack the bright-line filter
(occasional wins, CNPJ splitting) but is more resilient to
adaptations that affect ranking density (rotation, threshold
capping). Retention is the fraction of baseline AUC preserved.\\par}
\\end{table}", body)
  writeLines(tex, path)
}
write_tex(out, tex_out)
cat(sprintf("[done] wrote %s and %s\n", file.path(out_dir, "adv_adapt_table.csv"), tex_out))
