...
organization rather than through a single invariant price
statistic
...
The boundary is honest, and it favors a structural reading. The
obvious threat is that cobidders simply participate more: they bid
in roughly $1.8\times$ as many tenders as other frequent losers.
Matching cobidders to non-cobidder frequent losers on participation
count (standardized difference $\valVMBalRaw\rightarrow\valVMBalMatched$,
$\valVMnMatched$ matched pairs) leaves the structural distinctness
intact and, for several dimensions, larger: product-portfolio
concentration ($d=\valVMHHId$), winner-pair spread
($d=\valVMRepeatd$), and the bid gap to the winner
($d=\valVMGapd$) all hold or strengthen. The distinctness is
therefore not a participation-volume artifact. Two things do not
survive matching, and we report them as limits: the within-firm
bid-dispersion moment collapses to $d=\valVMSdGapd$
($p=\valVMSdGapP$), and a battery of bid-timing moments---revision
intensity, inter-bid interval, last-bid position, engagement
span---returns no significant difference (all $p\geq\valVMTimeMinP$).
The cobidder bid-conduct distinctness is thus a single channel
(proximity to the winning bid), not a multi-channel behavioral
signature. The within-stratum quadrant heterogeneity is likewise
largely a volume effect: the largest cell drops from
$\valMatchedHetLLUnmatched$ ($p=\valMatchedHetLLUnmatchedP$) to
$\valMatchedHetLLMatched$ ($p=\valMatchedHetLLMatchedP$) once volume
is matched away. The structural distinctness is robust; the
causal-mechanistic reading is the part the data do not support.
\ref{app:volume_matched_submission} reports the full volume-matched
audit.
