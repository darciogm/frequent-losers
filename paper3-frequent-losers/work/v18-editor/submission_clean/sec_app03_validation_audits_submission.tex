...
computed via the DeLong method). The permutation answers
the exposure concern directly: the frequent-loser concentration
near CADE defendants is larger than participation volume alone
predicts.
...
The opportunity set is defined from the award layer, using the
procurement environments in which a firm could have appeared
with a CADE anchor. The construction uses item, modality, year,
and buyer/procuring-unit information where the data provide
common support. Observations without the information needed to
place a firm in a comparable opportunity set are not used for
that comparison. The retained support is therefore the subset in
which firms can be compared to others facing similar chances to
encounter direct CADE defendants in routine procurement records.

The result is reported as an exposure-discipline check rather
than as a new liability measure. Its interpretation is limited:
it reduces the concern that the cobidder concentration is only a
mechanical byproduct of participation in defendant markets. It
does not prove agreement, identify cartel members, estimate
damages, or establish that all market-opportunity confounding
has been removed.
...
Firm &
Comparable opportunity sets using item, modality, year, and buyer/procuring-unit support &
Reported in text; see Appendix D.2 &
...
and ``withheld'' describe the identifying discipline each audit
