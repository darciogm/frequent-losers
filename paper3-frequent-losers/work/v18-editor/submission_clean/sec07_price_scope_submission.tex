...
losses. Price evidence is still useful, but it plays a narrower
role. It asks whether the environments prioritized by the
award-layer screen look economically non-neutral, and it helps
define where that interpretation must stop. Cartel detection,
liability, and overcharge measurement are related but distinct
tasks: overcharge measurement has its own evidentiary tradition
\citep{bryant1991price,connor2007price}, while cartel economics
and detection treat screening as a separate enforcement problem
...
The section uses price results in that limited way. It first
reports the broad descriptive price imprint associated with
frequent-loser presence. It then explains why overlap and segment
results narrow the scope of that imprint, especially where the
sign reverses outside high-value procurement. The final
implication is institutional: the award-layer signal is best used
as a trigger for richer evidence collection, not as a stand-alone
measure of harm. That boundary is what makes the screening claim
stronger for enforcement design rather than weaker.
...
The modality split is useful because it disciplines the screening
interpretation. If frequent losers were only mechanical quorum
...
pregão, the electronic-auction environment where repeated
loser-side positioning can be deployed without the same formal
invitation structure. This contrast supports the view that the
screen is not merely picking up a procedural artifact. It still
does not identify a causal procurement-price effect.
...
effects, cartel markups, or damages estimates.
\end{tablenotes}
\end{threeparttable}
\end{adjustbox}
\end{table}
...
causal effects, overcharges, or damages.
\end{tablenotes}
\end{threeparttable}
\end{adjustbox}
\end{table}
...
boundaries. The price evidence is consistent with economic
relevance, but it does not transform an award-layer statistic
into proof of agreement or harm. The correct institutional use is
staged: award records rank where costly bid-level and documentary
evidence should be assembled; bid records, case documents, and
legal process then determine whether the conduct supports an
antitrust case.

This distinction also clarifies portability. The BEC estimates
should not be exported as numerical external validity for other
jurisdictions. What travels is the information problem. Many
...
The screen should also be deployed as an adaptive queue rather
than as a static legal trigger. Any administrative rule can
