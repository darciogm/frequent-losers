...
% CO-AUTHOR EDIT [v21 minor]: state the range implied by the four displayed
% coefficients (exp of 0.0382 and 0.0933), not the across-estimator envelope macro.
Across these descriptive specifications the coefficient runs from
$\valTabPricesColFour$ (convite) to $\valTabPricesColThree$
(pregão), an implied price gap of roughly $+3.9\%$ to $+9.8\%$.
...
% CO-AUTHOR EDIT [v21 minor]: report the raw->FE attenuation honestly.
The raw cross-cell coefficient ($\valSelTestCoefRaw$) attenuates
by roughly $85\%$ to $\valSelTestCoefFullFE$ once the five cell
dimensions are absorbed, so most of the apparent selection is
cell composition; the surviving component is positive,
significant, and robust to partialling out each cell-dimension
marginal effect.
