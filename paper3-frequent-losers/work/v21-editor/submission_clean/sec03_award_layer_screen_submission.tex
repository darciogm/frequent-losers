...
% CO-AUTHOR EDIT [v21 B2]: motivate the separation with a role-allocation
% argument (kept as MOTIVATION, not a theorem; adversarial verification showed a
% clean derivation is tautological, contradicts D2, and is rejected by D4).
% Strictly win/lose space; no price; no quorum/modal comparative static.
The premise $s\perp\mathbf{1}[i\in D]$ is an identifying
assumption that a role-allocation argument makes plausible.
Suppose rent capture requires \emph{winning}---a ring earns the
inflated margin only on tenders it wins---that cover
participation is comparatively cheap, and that members differ in
the cost of delivering a contract. A rent-maximizing ring then has
reason to specialize: members who can deliver cheaply are assigned
to win and capture the rent, while members who cannot are deployed
to lose, supplying the appearance of competition without the cost
of performance. Because rent capture requires winning, the
principals a case ultimately targets tend to carry positive win
rates and leave the zero-win stratum on which the index is
defined, while the firms that remain in it are disproportionately
the assigned losers. Attention and liability separate because the
two roles sit on opposite sides of the win margin.

We offer this as motivation, not as a theorem, and keep it
strictly to role assignment: it is silent on prices and
within-tender conduct, and it does not identify cartel
organization. The separation it suggests is only approximate,
because rings are thin, capacity-constrained, and may rotate wins
precisely to blunt such a screen, leaving some principals in the
zero-win stratum. That is what the data show: the index is
near-random, not exactly random, against direct defendants
($\mathrm{AUC}=\valAUCdirectStd$), and $\valDirectShareAL\%$ of
direct defendants are themselves always-losers. We therefore
retain $s\perp\mathbf{1}[i\in D]$ as an assumption that the
role-allocation logic renders plausible and the data show holds
approximately, not as a derived result.
