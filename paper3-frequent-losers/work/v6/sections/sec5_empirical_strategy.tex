...
bid-coordination tests (Section~\ref{sec:bajari_ye}) and an RDD
exploiting the convite minimum-bidder threshold
(Section~\ref{sec:rdd_convite}).
...
We estimate the mixture model specified in
Section~\ref{sec:likelihood} using the 40 million bid-level
observations. The estimation proceeds in two stages.

\paragraph{Stage 1: Genuine bid distribution.}
We estimate the genuine bid parameters $(\boldsymbol{\beta},
\sigma_g^2)$ from the 37.5 million non-FL bids using maximum
...
completes in approximately [ESTIMATE] hours.
...
% ── 5.5 Convite Minimum-Bidder RDD ───────────────────────────────

\subsection{Convite Minimum-Bidder RDD}
\label{sec:rdd_convite}

Prediction~\ref{pred:minbidder_v6} implies a discontinuous increase in
cover-bidding incentives at the R\$80,000 threshold where procurement
modality switches from preg\~ao (no minimum-bidder requirement) to
convite (minimum three bidders). We exploit this threshold using a
regression discontinuity design.

\paragraph{Specification.}
The running variable is the estimated procurement value $V_t$. The
treatment is convite modality ($D_t = \mathbf{1}[V_t \leq 80{,}000]$).
We estimate:
\begin{equation}
  y_t = \alpha + \tau \cdot D_t + f(V_t) + \beta \cdot \text{FL}_t
  + \delta \cdot (D_t \times \text{FL}_t) + \varepsilon_t
  \label{eq:rdd}
\end{equation}
where $f(V_t)$ is a local-linear polynomial in the running variable,
fitted separately on each side of the cutoff, and $\delta$ captures the
differential FL--price effect under convite's minimum-bidder constraint.
Under Prediction~\ref{pred:minbidder_v6}, $\delta > 0$: the FL--price
association should be discontinuously larger just below R\$80,000 where
the minimum-bidder rule binds.

\paragraph{Bandwidth.}
We select the bandwidth using the \citet{calonico2014robust} MSE-optimal
procedure, implemented via the \texttt{rdrobust} package in R. We report
results at the optimal bandwidth and at half and double the optimal as
sensitivity checks.

\paragraph{Density test.}
A manipulation test \citep{cattaneo2020simple} checks whether the
running variable exhibits bunching at R\$80,000, which would indicate
strategic manipulation of procurement values to select modality. If the
density is smooth at the cutoff, the RDD provides locally valid causal
identification of the minimum-bidder effect on cover bidding.

\paragraph{Limitations.}
The RDD identifies a local treatment effect at the R\$80,000 boundary.
The reference price (which determines procurement value) may be
imprecisely measured, introducing fuzzy-RDD considerations. We report
both the sharp and fuzzy specifications.
