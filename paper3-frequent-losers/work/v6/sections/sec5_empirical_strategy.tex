...
completes in approximately 2--4 hours. % REVISION-V6: estimated from 100-rep pilot
...
% a discontinuity at R$80K: the convite rate is ~100% on both sides of
% the threshold in the relevant value range, because modality choice in
% BEC depends on procurement type (goods vs. services), not solely on
% contract value. Without a first-stage discontinuity in modality
% assignment, the RDD is not feasible with BEC data.
