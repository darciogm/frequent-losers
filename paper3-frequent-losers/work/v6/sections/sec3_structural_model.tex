...
We develop a structural model of cover bidding in procurement auctions
that formalizes the cover-bidder deployment decision, derives equilibrium
cover-bid distributions, and generates testable predictions that map
directly to our empirical strategy (Section~\ref{sec:empirical}). The
model has three aims: (i)~to provide a game-theoretic foundation for the
frequent-loser screen, replacing the descriptive taxonomy of prior
versions; (ii)~to derive a structural likelihood that separates cover
bids from genuine bids in the data; and (iii)~to generate the
market-selection prediction---that cover bidding concentrates in
competitive markets---which emerges endogenously from the cartel's
optimization rather than being imposed by assumption.
...
\boldsymbol{\gamma})$ by maximum likelihood using the 40 million
bid-level observations. Standard errors are computed via parametric
bootstrap (500 replications, parallelized across 16 CPU cores). The
genuine-bid parameters $(\boldsymbol{\beta}, \sigma_g^2)$ are estimated
from the 37.5 million non-FL bids; the cover-bid parameters
$(\delta \text{ or } \sigma_c)$ are estimated from the 2.5 million FL
...
The model generates six testable predictions, each mapped to a specific
...
\begin{prediction}[Minimum-bidder discontinuity]
\label{pred:minbidder_v6}
The FL--price effect is discontinuously larger under convite (where
$\underline{n} = 3$) than under preg\~ao (where $\underline{n} = 0$),
particularly for tenders near the R\$80,000 modality threshold.
\emph{Test:} RDD at the convite/preg\~ao threshold
