...
The empirical analysis has three parts. We first estimate the
price association using OLS with high-dimensional fixed effects,
temporal cross-fitting, and matching
(Section~\ref{sec:reduced_form})---these are the primary estimates, all
resting on selection on observables. We then check the screen's
detection performance against CADE cartel convictions and benchmark
it against bid-level screens
(Section~\ref{sec:detection_validation}). Finally, we present supporting diagnostics that assess whether the
screen's behavior is coherent with cover bidding, drawing on
structural estimation (Section~\ref{sec:structural_estimation}),
Bajari--Ye bid-coordination tests (Section~\ref{sec:bajari_ye}),
and network-split heterogeneity
(Section~\ref{sec:empirical_network}). These diagnostics check
coherence; they do not identify the mechanism.
...
Under the framework's first implication (Table~\ref{tab:predictions}), $\hat{\beta} > 0$ for prices.
...
Appendix~\ref{app:robustness}), raising exclusion-restriction
...
(Section~\ref{sec:cade}). This provides an initial check that FL presence identifies
