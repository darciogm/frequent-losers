...
prices supports the FL screen's empirical validity without requiring a
...
\paragraph{Threats to the exclusion restriction.} % REVISION-V5: expanded into subsections per referee minor 13
...
Three threats merit detailed discussion.

\subparagraph{Threat 1: Correlated demand shocks.}
...
(Table~\ref{tab:iv_panelc} in Appendix~\ref{app:robustness}). However,
the 2SLS estimate under this specification is 3.64---an implausibly
large markup that we attribute to weak-instrument amplification rather
than a credible price effect (see Section~\ref{sec:results_iv}).

\subparagraph{Threat 2: Coordinated cartel deployment.}
If a cartel coordinates FL deployment across PBUs simultaneously, the
LOO instrument captures coordinated entry rather than independent supply
variation. We address this by testing balance:
Table~\ref{tab:iv_balance} in Appendix~\ref{app:robustness} regresses
pre-determined observables on the instrument with item, year, and PBU
FE. All four observables are statistically significantly predicted by
the instrument ($p \leq 0.013$), though standardized effects are below
$0.03\sigma$. At $N > 1.5$ million, statistical significance is
uninformative; the relevant question is whether the imbalance is
economically meaningful. The pattern of significant imbalance across
\emph{all four} observables is more concerning than any individual
coefficient and is consistent with---though does not prove---systematic
violation of the exclusion restriction.

\subparagraph{Threat 3: Enforcement variation.}
If enforcement intensity varies across PBUs and correlates with both FL
...
We then split the treatment into two indicators---concentrated-market FL
and competitive-market FL---and test whether the price effect varies by
market structure.
