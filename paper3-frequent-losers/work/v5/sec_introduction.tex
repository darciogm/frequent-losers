...
\begin{description}
  \item[H1 (Screening marker):] FL presence is a useful \emph{screening
    marker} for potential collusion---it correlates with price anomalies
    and overlap with known cartel participants, regardless of whether
    every FL firm is itself a cover bidder.
...
\noindent Our primary contribution is to show that FL presence is a % REVISION-V5: reframed H2 as suggestive per referee condition 1
robust and implementable screening marker for bid rigging (H1). We then
present evidence from multiple empirical strategies---instrumental
variables, co-bidding network analysis, and bid-level
diagnostics---that is \emph{consistent with} a cover-bidding
mechanism (H2), while acknowledging that no single strategy
conclusively establishes the causal link. We do not claim to identify
all cartels in the sample, estimate structural overcharges, or measure
aggregate collusion welfare losses. The FL screen complements existing
tender-level forensic tools \citep{imhof2019detecting,
wallimann2023wild} with a \emph{firm-level} criterion that is
...
purchasing-unit fixed effects. External validation against CADE
...
\emph{Second}, we present suggestive evidence for the causal mechanism % REVISION-V5: reframed IV + network as suggestive per referee M1/M4
...
($F = 396$) and the IV estimate of a 21\% price markup (LATE) exceeds
the OLS estimate, though the IV is sensitive to the fixed-effects
structure and should be interpreted with caution. Co-bidding
network analysis reveals that the FL--price effect is concentrated in
competitive markets (low winner concentration), consistent with cover
bidding being most profitable where genuine competition exists, though
alternative explanations involving selection cannot be fully excluded.

\emph{Third}, we apply \cites{bajari2003deciding} framework to test % REVISION-V5: honest Bajari-Ye framing per referee M2
for bid coordination using the FL classification as a natural partition.
FL bid residuals violate exchangeability (Kolmogorov--Smirnov
$D = 0.15$, $p < 0.001$). The conditional independence test yields
a larger pairwise product for FL (5.16) than non-FL (2.21) under
baseline fixed effects; however, under tender fixed effects that absorb
all tender-level confounds, the FL pairwise product (0.38) falls below
the non-FL product (0.86). We interpret the Bajari--Ye results as
evidence of systematic heterogeneity between FL and non-FL bidders,
consistent with---but not conclusive proof of---cover bidding.
