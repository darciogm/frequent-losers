...
cobid_path <- "work/v22-editor/outputs/cache/canonical_cobidders_broad.csv"  # canonical broad AL label (651)
fls_path   <- file.path(data_dir, "firm_loss_stats.parquet")
ftm_path   <- file.path(data_dir, "firm_tender_map.parquet")
out_dir    <- "work/v22-editor/outputs/diagnostics/adversarial_adaptation"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
tex_out    <- "work/v22-editor/outputs/tables/appendix/tab_adversarial_adaptation.tex"
...
cobid <- fread(cobid_path)[broad_cobidder == 1L]
cobid_cnpjs <- unique(as.character(cobid[["códigofornecedor"]]))
...
macros_path <- "work/v22-editor/submission_clean/values_adversarial.tex"
