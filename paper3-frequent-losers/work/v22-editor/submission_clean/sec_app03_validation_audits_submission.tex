...
This appendix gives the logic of the opportunity and timing audits and
states the interpretation each one licenses; the main text carries the
headline numbers, and Online Supplement~C holds the full grids behind
every audit summarized here.
...
statistically detectable but operationally negligible: the nested
DeLong increment over the exposure-only model is only $+0.010$ AUC
($p=0.013$)---a real but tiny residual, well below any AUC gain
that would change a forensic-prioritization decision---while the
...
held fixed, the residual ordering is \emph{small and not
...
better ($0.507$). The $+0.010$ increment is the only point estimate
in the battery that is both positive and conventionally significant;
read on its own terms it is statistically real ($p=0.013$) but
operationally negligible---a one-percentage-point AUC gain over an
exposure-only model is far below any margin that would reorder a
forensic queue. We do not lean on its $p$-value, and we note that the
opportunity-adjusted battery comprises many correlated tests
(nested increment, within-stratum AUC, CEM-matched AUC, two
exposure-preserving permutations, and the intensity-restricted
re-run); under a Benjamini--Hochberg correction over this family the
$p=0.013$ increment is the only test that survives at the $5\%$
level, and the headline within-stratum and permutation nulls are
unaffected because they are non-rejections to begin with. The score does rank the binary excess-contact
...
\emph{negative} ($-0.017$). We therefore report no operationally
relevant residual ordering net of opportunity; the only positive,
conventionally significant quantity is the $+0.010$ nested
increment, which is statistically real ($p=0.013$) but too small to
move a prioritization decision, and which the permutation tests do
not corroborate as a robust signal.
...
The restriction sharpens the negative conclusion rather
...
exposed subsample; $0.956$ unconditionally), again exceeding
the score's; the within-opportunity-stratum AUC remains at chance
...
power.} Because a negative verdict can be manufactured as
easily as an inflated one, we turn the audit on its own machinery.
...
stratifies on opportunity deciles; a robustness check that
instead rebuilds the strata from the discrete expected-contact
cells of \ref{app:market_opportunity_submission} leaves the
within-stratum score AUC in the range
$\valArmorWithinMedium$--$\valArmorWithinStrict$ across
granularities (coarse $\valArmorWithinCoarse$, medium
$\valArmorWithinMedium$, strict $\valArmorWithinStrict$). The
sequence is non-monotone---it dips at the medium definition and
rises in the strictest cells---and retained support shrinks from
$15{,}246$ to $615$ firms across that range. The strict-cell
$\valArmorWithinStrict$ is not evidence of a residual: it rests on
only $615$ retained firms (16{,}295 within-cell pairs), where the
$O_i$ positive control itself falls to $0.740$, signalling that the
strictest cells are too thin to discriminate reliably; under the
same coarse and medium cells the score stays at or below chance.
Stratifying instead on the \emph{label-blind} expected
...
content. These label-frozen counts are distinct objects from the
strict-holdout rows in Table~\ref{tab:strict_holdout_submission}:
the $\valArmorFrozenPool$ pool comprises all frozen-always-loser
incumbents with any pre-2017 history, whereas the
``training-AL \& test-active'' row intersects that pool with
test-window activity ($11{,}351$ firms, $312$ positives under the
full-window label); $\valArmorFrozenProspN$ counts firms whose
\emph{first} defendant contact arises in $2017$--$2019$, while
$\valArmorFrozenRetroN$ counts contact within $2009$--$2016$
only, and the two need not nest the full-window label counts.

\subsection{The Over-Crediting Bias as an Estimable Object}
\label{app:overcrediting_submission}

\textit{Purpose.} The opportunity audit above shows \emph{that} the
raw award score over-credits defendant contact. This subsection
characterizes \emph{why}, and turns the caveat into a diagnostic a
practitioner can compute before opening a single bid file.

\textit{The size-bias mechanism.} The cobidder label is
contact-defined: a firm is a positive if it shares at least one
tender-item with an adjudicated defendant. The probability of such a
contact is mechanically increasing in a firm's participation volume
$T_i$,
\begin{equation}
...
density $p_g$ of \ref{app:expected_contact_submission}). The award
score is monotone in the same $T_i$ by construction
($\log(1{+}T_i)$). \emph{Both the score and the label therefore load
on participation volume.} A ranking by the score orders the label not
because loss intensity is behaviorally informative, but because each
is a function of $T$. This is the same fact the within-stratum null
($\valExpWithinAUC$, $\approx$ chance) and the label-blind exposure
ranking ($\valArmorExpLB$) establish empirically; here we give its
analytic shape.

Write $F_T$ for the distribution of $T$ in the candidate pool. Under
the rare-target linearization in \eqref{eq:contact_sizebias} the
positive class is $T$ weighted by $\pi T$, i.e.\ the
\emph{size-biased} distribution of $T$, while the negative class is
approximately $F_T$ itself. Since the score is monotone in $T$, the
raw ROC area is the size-bias stochastic gap
\begin{equation}
...
volume channel fixed (the within-opportunity-stratum / label-blind
value); it is the genuine-signal floor, which the audits put at
$\approx$ chance. The over-crediting bias is the gap
...
We deliberately state \emph{signs}, not a magnitude formula:
off the rare-target linearization, with the empirical heavy-tailed
$F_T$ and a multi-cell opportunity adjustment, the magnitude
$g(\cdot)$ is not closed-form, and a formula would be spurious
precision. We characterize the magnitude by a transparent simulation
instead.

\textit{Simulation.} Figure~\ref{fig:inflation_surface} maps $\Delta$
over a grid in $\mathrm{CV}(T)$ and the base rate. The simulation is
\emph{synthetic}: it draws $T$ from a Gamma family indexed by
$\mathrm{CV}(T)$ (the Poisson-mixing family of
\ref{app:framework_scope}), assigns the contact label by
\eqref{eq:contact_sizebias} at a target base rate, scores firms by a
noisy $\log(1{+}T)$ calibrated once so the BEC coordinate reproduces
its observed raw area, and compares the raw ROC area to the
...
adjudicated base rate. Across the grid the simulated surface confirms
both signs of Proposition~\ref{prop:inflation}: $\Delta$ rises
monotonically in $\mathrm{CV}(T)$ (from $\approx 0.03$ at
$\mathrm{CV}=0.5$ to $\approx 0.34$ at $\mathrm{CV}=5$) and falls in
the base rate once the target leaves the rare regime.
...
\textit{The sufficient statistic.} The practitioner deliverable is
that $\mathrm{CV}(T)$ of the candidate pool, computable from award
data alone before any bid file is opened, together with the target
base rate, bounds how much of any raw area is mechanical. A high
dispersion of participation volume is a warning that a contact-anchored
validation will over-credit a volume-loaded score; the correct
response is the within-stratum / label-blind comparison, not the raw
area. We emphasize that this is a \emph{diagnostic}, not a fix: the
correction here reveals the genuine floor to be $\approx$ chance
(\ref{app:control_function_submission}); it does not rescue the screen.

\textit{BEC and federal as two points on one curve.} The two
platforms are not ``the same null twice''---they are a second
coordinate on the inflation surface. BEC has
$\mathrm{CV}(T)=2.79$ and an always-loser base rate of $3.9\%$
($651/16{,}843$); its raw area is $0.761$, its within-stratum floor
$\valExpWithinAUC$, so $\Delta\approx0.29$. The federal platform has a
higher dispersion ($\mathrm{CV}(T)=3.79$) and a roughly seven-fold
lower base rate ($0.5\%$, $195/35{,}943$); its raw area is
$\valFedAudRawAUC$ and its within-stratum floor $\valFedAudWithinAUC$,
so $\Delta\approx0.28$, comparable to BEC. The lower federal base rate
is precisely why its exposure-only model \emph{edges past} the raw
score ($\valFedAudExpOnlyAUC > \valFedAudRawAUC$) where the BEC
exposure-only model does not: at a rarer target the size-biasing is
nearly complete, so a pure opportunity ranking captures essentially
all of what the raw score captures. The within-stratum floor is
$\approx$ chance on both platforms. This is the generalizing content:
the inflation \emph{law}---the first-stage deflation and the
label-blind exposure ranking---replicates at full power across the two
platforms; only the residual within-stratum floor is power-bounded
federally (below an AUC of roughly $0.55$ at $N^{+}=195$), and we keep
the two apart.
...
\textit{Result.} The reading is deliberately cautious. Only a
...
anchoring the operational signal. The case-composition inference
rests on a small number of adjudicated cartels---twelve matched
CADE cases, of which six are informative for the case-grouped
exercises---so the effective number of clusters at the natural
unit of treatment (the case) is small. The randomization
inference is clustered at the finer item-group $\times$ year
level, which does not manufacture case-level independence: under
the honest few-effective-clusters reading, $p=\valClusterRIp$ and
the coverage $p=\valClusterRIcovP$ are directionally informative
rather than exactly size-controlled, and the paper leans on the
corroborating anchor-agnostic battery rather than on any single
clustered $p$-value.
