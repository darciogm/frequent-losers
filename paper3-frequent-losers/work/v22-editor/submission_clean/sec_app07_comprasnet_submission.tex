...
which is substantially larger than the BEC cobidder set
(\valMainCobidders) even though the federal direct-defendant set is
...
\valFedAucContinuous{} on the same federal pool, above any
...
cobidders from non-cobidders above chance (AN-004). The continuous
loss-intensity classifier orders firms at least as well as the
binary classifier on both panels, with the DeLong direction
preserved. The structural boundary against
