...
before proof exists. Using S\~ao Paulo's BEC platform and CADE
...
residual ordering net of opportunity is marginal.
Re-running the same audit on a second federal platform with
partially overlapping legal anchors returns the same verdict, so
what transfers is the audit protocol, not a deployable score. The
contribution is the organizational result---the award-to-bid
recovery decision is a sequential cost-recall problem whose
frontier, not any cutoff, is the design object---and a disciplined
audit protocol demonstrated across two platforms. Liability remains
in the richer evidentiary record.
\end{abstract}
