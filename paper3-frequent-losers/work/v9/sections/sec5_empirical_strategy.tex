...
\paragraph{Instrumental variable (measurement-error correction).}The binary FL indicator contains misclassification noise: firms near
...
instrument primarily to correct for this attenuation:
\begin{equation}
  Z_{kgt} = \sum_{j \neq k} \mathbf{1}[\text{FL firm active at PBU } j
  \text{ in group } g, \text{ year } t]
  \label{eq:iv_v6}
\end{equation}
The instrument integrates over firm-level misclassification by
exploiting aggregate FL supply variation at other purchasing units in
the same product market and year---a supply-side exclusion restriction
in the spirit of \citet{goldsmithpinkham2020bartik}. We treat
$\hat{\beta}_{\text{IV}}$ as a measurement-error diagnostic rather
than a primary estimator, for three reasons: (i)~balance tests reveal
systematic imbalance across observables (all standardized differences
$< 0.03\sigma$), which is negligible for measurement-error correction
but would matter for clean causal inference; (ii)~the LATE
interpretation restricts the estimate to complier tenders whose FL
status switches in response to aggregate supply variation; and
(iii)~if cartels operate across PBUs within the same product market,
the instrument may capture cartel intensity rather than
misclassification alone, biasing the IV upward beyond
measurement-error correction.

\paragraph{IV validity concerns.}
Balance tests (Table~\ref{tab:iv_balance} in Appendix~\ref{app:robustness}) show that all four observables are significantly predicted by the instrument. Standardized differences are individually small ($< 0.03\sigma$), but the pattern of systematic imbalance across all observables is consistent with---though does not prove---violation of the exclusion restriction. The \citet{goldsmithpinkham2020bartik} Rotemberg decomposition is infeasible in our setting ($> 200{,}000$ item-year shares). We therefore triangulate through matching (CEM, IPW) and sensitivity analysis (\citealt{cinelli2020making}, $RV = 17.5\%$). The IV is a diagnostic: it indicates that the OLS is conservative, but we do not include it in the headline range. Our primary estimates are OLS (0.064), cross-fit (0.036), and matching (0.055--0.077).
