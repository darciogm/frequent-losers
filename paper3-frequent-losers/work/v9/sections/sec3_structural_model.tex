...
We discuss identification of each parameter block in turn.
...
in the same tender. This permits estimation of~\eqref{eq:genuine_dist}
on the 23.2~million non-FL losing bids without selection correction.

\paragraph{Cover-bid parameters $(\delta$ or $\sigma_c, \epsilon)$.}
Conditional on the winning bid $b^*_t$ (observed in each tender),
the cover-bid distribution is identified from the 189,381 FL losing
...
among FL bids. Model selection between regimes uses BIC on the
tender-level log-likelihood~\eqref{eq:loglik}.

\paragraph{Detection probability $\theta_k$.}
This parameter is not point-identified from the bid data alone.
We bound it using cross-PBU variation in oversight capacity, proxied
by procurement volume: larger PBUs with dedicated procurement staff
have higher $\theta_k$. The comparative static $\partial m^* /
\partial \theta_k < 0$ (Proposition~\ref{prop:optimal_m}) generates
a testable implication---FL presence decreases with PBU size---which
we evaluate in the oversight heterogeneity analysis
(Section~\ref{sec:robustness}).

\paragraph{Cartel markup.}
The markup $b^* - c_{j^*}$ is not separately identified by the
structural model: we observe $b^*$ but not $c_{j^*}$. The
reduced-form OLS coefficient captures the \emph{average price
difference} between FL-present and FL-absent tenders within item,
year, and PBU (Section~\ref{sec:results_structural}). The structural
model's contribution is characterizing the cover-bid \emph{mechanism}
(regime identification, distributional parameters, market-selection
prediction), not providing an independent markup estimate.
