...
BEC reference prices are set administratively by purchasing units, % V7-T3: institutional detail + bad-control connection
typically as the median of three supplier quotations or the average of
recent comparable tender prices. Because the reference price is public,
it serves as a coordination anchor for cartels: the designated winner
bids just below $r$, and cover bidders bid above the winner but within
a plausible range of $r$.

If cartels use cover bids to inflate reference prices for future
tenders (e.g., by submitting high bids that enter the quotation pool
for subsequent procurements), winners in FL-present tenders should bid
closer to the reference price. We regress
$\log(p_{\text{negot}} / p_{\text{ref}})$ on FL presence. The
coefficient is $-0.041$ ($p < 0.01$), meaning that winning bids in
FL-present tenders are 4\% closer to the reference price. This is
consistent with cartels calibrating the winning bid near $r$ while
cover bidders submit bids that do not undercut it
\citep{marshall2012economics}. This finding also informs the
bad-control concern in Section~\ref{sec:robustness}: because reference
prices may incorporate prior cartel-inflated bids, conditioning on
$\log r$ in the price regression removes part of the treatment effect
and may induce collider bias.
