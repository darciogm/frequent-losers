...
\footnote{Under BEC's procurement rules, the reference price is % V7-T3: institutional detail on reference price formation
...
% CO-AUTHOR EDIT: THEORETICAL INSIGHT 1 [T2.3a]
\paragraph{Theoretical implication: cover bidding as strategic complement.}
Proposition~\ref{prop:market_selection} yields a non-obvious
prediction: cover bidding is a strategic complement to genuine
competition, not a substitute. Cartels deploy more cover bidders in
competitive markets (low HHI), where manufactured competition is most
valuable for sustaining supra-competitive prices without attracting
suspicion. This reverses the standard detection heuristic that
collusion concentrates in concentrated markets
\citep{marshall2012economics}.

% CO-AUTHOR EDIT: THEORETICAL INSIGHT 2 [T2.3b]
\paragraph{Theoretical implication: cover bidding reduces bid dispersion.}
Regime~2 (Proposition~\ref{prop:regime2}) predicts that cover bids
exhibit \emph{lower} dispersion than genuine bids
($\sigma_c < \sigma_g$), because coordinated cover bidders cluster near
a focal point. This finding reverses a common assumption in the cartel
detection literature \citep{imhof2019detecting}, which associates
collusion with \emph{high} bid dispersion. Cover bidding, by contrast,
produces lower dispersion relative to genuine competition---implying
that variance-based screens may yield false negatives for cartels that
employ cover bidders.
