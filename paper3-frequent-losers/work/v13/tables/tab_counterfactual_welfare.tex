% tab_counterfactual_welfare.tex
% Policy counterfactuals from the structural cover-bid model.
% Built by work/v7/scripts/counterfactual_welfare.R; rerun produces the same numbers.

\begin{table}[htbp]
\centering
\caption{Policy Counterfactuals from the Structural Model}
\label{tab:counterfactual_welfare}
\small
\begin{tabular}{lp{6.5cm}rr}
\toprule
Counterfactual & Description & OLS Estimate & IV Estimate \\
\midrule
\multicolumn{4}{l}{\textit{Panel A: Remove Minimum-Bidder Requirement (Convite)}} \\
\\[-2pt]
 & Convite FL-present tenders (total) & \multicolumn{2}{c}{121,338} \\
 & Constraint-binding tenders ($n_{\text{genuine}} < 3$) & \multicolumn{2}{c}{47,611 (39.2\%)} \\
 & Pure cover-bidding tenders ($n_{\text{genuine}} = 0$) & \multicolumn{2}{c}{28,561 (23.5\%)} \\
 & & & \\[-4pt]
 & Annual welfare gain (constraint-binding) & R\$7.4M & R\$21.1M \\
 & Annual welfare gain (pure cover-bidding) & R\$4.0M & R\$11.5M \\
\midrule
\multicolumn{4}{l}{\textit{Panel B: Optimal Screening Threshold (Deterrence = 50\%)}} \\
\\[-2pt]
 & Net welfare at R\$100K/investigation & \multicolumn{2}{c}{R\$134.7M} \\
 & Optimal IQR multiplier at R\$100K & \multicolumn{2}{c}{1.6$\times$ IQR} \\
 & Firms flagged at optimal threshold & \multicolumn{2}{c}{2,537} \\
 & Net welfare at R\$1M/investigation & \multicolumn{2}{c}{R\$-634.5M} \\
\midrule
\multicolumn{4}{l}{\textit{Panel C: Welfare Gain from Increasing Detection Probability ($\theta$)}} \\
\\[-2pt]
 & Estimated elasticity $\partial \log m^* / \partial \log \theta$ & \multicolumn{2}{c}{0.316} \\
 & Welfare gain per 10\% $\uparrow\theta$ & R\$23.5M & R\$66.9M \\
 & Welfare gain per 50\% $\uparrow\theta$ & R\$117.5M & R\$334.3M \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item \textit{Notes:} Panel A reports the welfare effect of removing the
  convite minimum-bidder requirement (Lei 8.666/93, Art.~22, \S7).
  Constraint-binding tenders are those with genuine bidders $n < 3$,
  where the cartel must deploy cover bidders solely to satisfy the rule.
  Pure cover-bidding tenders have $n=0$.
  Annual estimates divide total-period savings by 10 years (2009--2019).
  Panel B reports optimal IQR-multiplier thresholds under different
  investigation cost budgets (baseline deterrence rate = 50\%).
  Panel C uses cross-PBU variation in oversight (PBU size quartiles as
  proxy for $\theta$) to calibrate the elasticity of $m^*$ with respect
  to detection probability. OLS: $\hat{\beta} = 0.064$;
  IV: $\hat{\beta} = 0.194$. All welfare figures in BRL (nominal).
  The IV estimate represents a LATE for complier tenders and corrects for measurement-error attenuation in the binary FL indicator; it is not a causal point estimate.
\end{tablenotes}
\end{table}
