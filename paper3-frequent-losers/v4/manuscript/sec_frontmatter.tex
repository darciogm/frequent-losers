...
IQR-based threshold. FL presence is associated with 4--9\% higher % CO-AUTHOR EDIT: was 6-9%, now 4-9% to include convite (3.8%)
negotiated prices in OLS specifications with item, year, and
purchasing-unit fixed effects. External validation shows that 7.1\% of
FL firms co-participate with CADE-convicted cartelists, 3.5 times the
rate expected by chance ($p < 0.001$, permutation test). A
leave-one-out instrumental variable exploiting FL supply at other
purchasing units yields a 21\% price markup (first-stage $F = 396$),
consistent with OLS attenuation from measurement error. Bajari--Ye
tests reject exchangeability and conditional independence for FL bid
residuals. Estimated welfare loss ranges from R\$734 million (OLS) to
R\$2.4 billion (IV).
\end{abstract}
