...
\noindent We present evidence that strongly supports H1 and provides
substantial---though not conclusive---evidence for H2. The distinction
matters for policy: even under H1 alone, the FL screen is operationally
valuable for targeting investigations; H2 additionally implies that
removing FL firms from the bidding pool would directly reduce prices.

\paragraph{Scope.}
We do \emph{not} claim that every FL firm is a deliberate cover bidder,
nor that the FL screen is the only or best tool for detecting bid
rigging. Rather, we establish FL presence as a \emph{useful screening
marker}---one element in a broader detection toolkit---that is
particularly valuable where bid-level data are unavailable.
...
\emph{First}, we establish the FL screen as a credible detection tool
(H1). FL-present tenders exhibit 4--9\% higher negotiated prices in OLS
specifications with item, year, and purchasing-unit fixed effects.
External validation against CADE (Brazil's competition authority)
convictions shows that 7.1\% of FL firms co-participate with convicted
cartelists---3.5 times the rate expected by chance after controlling
for participation intensity ($p < 0.001$, permutation test). The
FL--price association persists after excluding all CADE-involved
markets, confirming that the screen detects anomalies beyond known
cartels.

\emph{Second}, we provide causal evidence consistent with H2 using a
leave-one-out instrumental variable (IV) strategy. The instrument
exploits supply-side variation in FL firms' availability: for each
purchasing unit, we measure FL activity at \emph{other} purchasing
