...
(Table~\ref{tab:iv_panelc} in Appendix~\ref{app:robustness}). Second, \emph{statewide cartel
...
independent supply variation. We address this by testing balance: % IJIO-R1: M2 — Balance test honest assessment
Table~\ref{tab:iv_balance} regresses pre-determined observables on the
instrument. The pattern of statistically significant imbalance across
all four observables is more concerning than any single coefficient;
we rely on the sensitivity analysis
(Section~\ref{sec:robustness}) and matching estimates to assess
whether this imbalance is sufficient to explain the price effect. Third, \emph{enforcement variation}: if
...
The first stage regresses $\log$ bid values on observable cost shifters % IJIO-R1: M3b — Section 6.4 first stage description corrected
...
effects. We report the first-stage $R^2$ and coefficient estimates in
