...
2. ~~**T3.1 opening paragraph**~~: RESOLVED — rewritten to lead with mechanism → FL → result (AUC 0.94) → policy.
3. ~~**AUC forward reference**~~: RESOLVED — AUC 0.94 now in opening paragraph, contributions, and comparison table.
4. ~~**Rationality test (T2.1)**~~: RESOLVED — replaced %-of-unit-price with fixed bidding costs (R$50/200/500), calibrated to BEC electronic platform.
5. ~~**Cover bid regression (T1.4)**~~: RESOLVED — v3/bid_level_analysis.parquet confirmed identical to main pipeline (39.96M rows, 2,735 FL firms, 41,444 total firms, same threshold 13.5). Generated by v3/code/04_cover_bid_flags.R from same source data.
6. ~~**Latent Class (T1.2)**~~: RESOLVED — added unsupervised LCA (flexmix, k=2) alongside existing supervised comparison. Cover bidder class contains 71.2% FL firms; Pr(cover|FL)=0.56. Partial but meaningful alignment.
7. ~~**Bibliography**~~: RESOLVED — all new citations verified in references.bib: asker2010leniency (L234), marshall2012economics (L254), imhof2019detecting (L121), bajari2003deciding (L24).
