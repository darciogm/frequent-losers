...
\emph{perto-mas-isolado} sobe de \valMortFOcrR{} para \valMortFOasR{} por 100 mil --- mas não inverte o
ordenamento: os três quadrantes de deserto permanecem \emph{abaixo} do quadrante
conectado, cujo SMR (\valMortNEsmr) é o mais alto da tabela.% src: mortality_crude_vs_agestd_by_quadrant.csv
No plano contínuo o padrão é o mesmo e, se algo, contrário à intuição: a
correlação entre a taxa padronizada e o burden de fluxo F1 é $\valCorrMortF$, e o
coeficiente de F1 permanece negativo ($\valBetaMortF$ óbito por 100 mil a cada
...
Quadrante & Munic. & Taxa bruta & Taxa padroniz. & SMR \\
          &        & (/100 mil) & (/100 mil)     &     \\
\midrule
Perto-mas-isolado (fluxo) & \valFOn & \valMortFOcr & \valMortFOas & \valMortFOsmr \\
Longe e isolado           & \valBOn & \valMortBOcr & \valMortBOas & \valMortBOsmr \\
Longe-mas-conectado       & \valDOn & \valMortDOcr & \valMortDOas & \valMortDOsmr \\
Conectado (referência)    & \valNEn & \valMortNEcr & \valMortNEas & \valMortNEsmr \\
\bottomrule
\end{tabular}
\par\smallskip
...
crua nacional (\valMortNatCrude/100 mil). Quadrantes definidos na
...
total bruta é de \valMortTotNE{} por 100 mil no quadrante conectado contra cerca
de \valMortTotDes{} nos de fluxo isolado ---,% src: bloco garbage/total-mort por quadrante
...
explicação fraca: a fração de óbitos R96--R99 é de \valGarbageLo\% a \valGarbageHi\% nos desertos
contra \valGarbageNE\% no conectado --- diferença pequena demais para gerar o gap
