% Secao - mobilidade medica (lado da oferta). Numeros = macros de values.tex.
...
estabelecimento $\times$ mês, 2015--2023 --- \valNvinc{} milhões de vínculos
médicos ---, identificando o médico pela família CBO de medicina (verificada
contra o registro no CRM) e deduplicando-o pelo Cartão Nacional de Saúde, presente
em 100\% dos registros.% src: 15/16; CNS_PROF; CBO 2231/2251/2252/2253 = CONSELHO 71
...
A Tabela~\ref{tab:medicos} é direta. Os municípios perto-mas-isolados têm
\valFOmed{} médico por mil habitantes contra \valNEmed{} no quadrante conectado
--- uma oferta uma vez e meia menor --- e o déficit é mais agudo onde mais
importa: especialistas (famílias cirúrgica e diagnóstico-terapêutica) são
\valFOesp\% do corpo clínico nos desertos de fluxo contra \valNEesp\% no quadrante
conectado, e apenas \valBOesp\% no deserto pleno.% src: physician_supply_panel / bloco especialista por quadrante
...
primária é semelhante em todos os quadrantes (\valFOprim{} médico de unidade
básica por mil habitantes nos perto-mas-isolados contra \valNEprim{} no
conectado), mas o médico hospitalar é escasso (\valFOhospd{} contra \valNEhospd)
e o especialista ainda mais (\valFOespd{} contra \valNEespd, e apenas \valBOespd{}
no deserto pleno).% src: 19_physician_setting / physician_setting_by_quadrant.csv
...
\midrule
Perto, deserto em fluxo & \valFOmed & \valFOesp & \valFOret & $\valFOnet$ \\
Deserto pleno           & \valBOmed & \valBOesp & \valBOret & $\valBOnet$ \\
Longe, conectado        & \valDOmed & \valDOesp & \valDOret & $\valDOnet$ \\
Sem deserto             & \valNEmed & \valNEesp & \valNEret & $\valNEnet$ \\
\bottomrule
\end{tabular}
\par\smallskip
...
A retenção anual de médicos é menor nos desertos (\valFOret{} contra \valNEret{}
no quadrante conectado), e o saldo migratório separa nitidamente os dois mundos:
os três quadrantes de deserto perdem médicos ano após ano --- $\valFOnet$ por mil
médicos nos perto-mas-isolados, $\valBOnet$ no deserto pleno --- enquanto o
quadrante conectado é o único que \emph{ganha} ($\valNEnet$).% src: physician_by_quadrant.csv; net_por_1000_med
...
município$\rightarrow$município (correlação de \valNetCorr{} sobre 54 mil pares
comuns; Figura~\ref{fig:redes_medicas}): médicos e pacientes deixam os mesmos
lugares por corredores parecidos.% src: physician_vs_patient_network.csv; r_log 0.455
...
\valTophubFO\% dos municípios perto-mas-isolados, contra \valTophubNE\% nos
conectados, e somente \valFracToHub\% dos médicos que partem rumam para o hub que
recebe a maior parte dos pacientes do município.% src: tophub_match_by_quadrant.csv
...
residência do profissional está preenchido em apenas \valUFMUNRESfill\% dos
vínculos, o que impede separar deslocamento pendular de migração efetiva ---;% src: UFMUNRES fill-rate 7.3%
