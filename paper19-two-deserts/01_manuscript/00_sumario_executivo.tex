...
% Headline number unico, repetido no abstract e na conclusao. Numeros = macros.
...
\medskip
\noindent\textbf{O que fazemos.} Para os \valNmunic{} municípios brasileiros
(2015--2022), confrontamos o deserto de \emph{distância} (km ao hospital ativo
mais próximo) com o deserto de \emph{fluxo} (burden efetivo de viagem, ponderado
por onde os pacientes de fato internam, construído de \valNaih{} milhões de
internações do SIH). O exercício é descritivo; nenhuma reivindicação causal é
feita.

\medskip
\noindent\textbf{Três números que o relatório sustenta.}
\begin{itemize}
\item As duas medidas correlacionam-se apenas moderadamente ($r=\valCorrFD$): a
  distância ao hospital mais próximo explica cerca de \valRtwoFD\% da variação no
  burden efetivo de fluxo. O paciente típico viaja \valMedGap~km \emph{além} do
  hospital mais próximo.
\item Dos municípios sinalizados como deserto por qualquer dos dois conceitos,
  \valPctDisagree\% o são por apenas um deles. Os mapas discordam sobre a maioria
  dos desertos.
\item \textbf{\valFOn{} municípios, onde vivem cerca de \valFOpop{} milhões de
  pessoas, têm um hospital por perto no mapa mas são desertos de fluxo} --- seus
  pacientes viajam, em média efetiva, \valFOf~km, e 90\% das internações deixam o
  município. São invisíveis a uma política de distância.
