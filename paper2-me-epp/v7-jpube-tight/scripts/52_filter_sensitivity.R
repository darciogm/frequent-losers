# S5 / task 52 -------------------------------------------------------
# Filter sensitivity da BNE decomposition (regime all-bidders, v3 final).
# Quatro configurações de corte:
#   baseline   : c_norm_clean ∈ (0, 3],  n_firms ≥ 2   (v3 final)
#   tight      : c_norm_clean ∈ (0, 2],  n_firms ≥ 3
#   very_tight : c_norm_clean ∈ (0, 1.5], n_firms ≥ 3
#   loose      : c_norm_clean ∈ (0, 5],  n_firms ≥ 2
#
# Janela temporal (18m) não é variada aqui — exigiria re-gerar
# bids_uh_cleaned com data_oc_numb preservado; fica como open risk.
...
filters <- list(
  ...
  loose       = list(c_up = 5.0, n_min = 2))

set.seed(seed_for_script(52))
all_res <- list()
...
for (i in seq_len(nrow(res))) {
  ...
  tex <- c(tex, sprintf(
    "%s & %s & %s & %.3f & %.3f & %+.4f & %.1f \\\\",
    r$pharma_lbl, r$filter, format(r$n_bids, big.mark = ","),
