# S6 / task 56 -------------------------------------------------------
# Bootstrap CI (B=500, cluster at auction) da welfare decomposition.
# Mesma estrutura do 51; resample auctions, recompute F_c (all-bidders),
# simula 500 auctions, calcula DWL_alloc, Δ_gov, MCPF_dist, total loss
# % de p_S1. Paralelização via mclapply (12 cores).
...
one_bootstrap <- function(bs_idx) {
  set.seed(seed_for_iter(56, bs_idx))
  ...
  for (ph in c(0, 1)) {
    ...
    sim <- simulate_welfare(en_pre$n_sme, en_pre$n_nonsme,
...
B <- 500
n_cores <- 12
log_step("56", sprintf("rodando B=%d em %d cores", B, n_cores), logf)
t0 <- Sys.time()
