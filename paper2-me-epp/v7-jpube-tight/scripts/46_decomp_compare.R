# S4 / task 46 -------------------------------------------------------
# Compara as quatro versões da decomposição (intensive vs entry share)
# num grid 2×2:
#   raw F_c × fixed-pool   (proxy do que v2 faz)
#   raw F_c × endogenous   (intermediário)
#   clean F_c × fixed-pool (intermediário)
#   clean F_c × endogenous (versão final v3)
#
# Mostra quanto cada melhoria metodológica muda o headline.
...
# Reaproveita a estrutura do script 45 mas com F_c raw vs clean. --
...
run_scenario <- function(fc_samples, entry_counts, endogenous = TRUE) {
  ...
  for (ph in c(0, 1)) {
    ...
    p_S2 <- simulate_auction(n_pre$n_sme,  0,
                              fc_sme_pre, fc_ns_pre)
    # S3: endógena usa N^SME_Post; fixed-pool mantém N^SME_Pre.
...
log_step("46", "rodando 4 combinações (all-bidders)", logf)
...
# Sensitivity: losers-only vs all-bidders na versão final
# (clean + endogenous). Reporta só no log — documenta a magnitude
# do bias MC3 para o memo.
set.seed(seed_for_script(46))
...
cat("\n--- sensibilidade MC3: clean+endog, losers-only vs all-bidders ---\n",
    file = logf)
sink(logf, append = TRUE)
sens_cmp <- rbind(
