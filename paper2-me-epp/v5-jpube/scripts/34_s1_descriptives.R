# ----------------------------------------------------------------------
# Consolidates the S1 handoff: descriptive statistics by stratum
# (modality × period × pharma × SME) that will support a decision on
# como as upcoming sprints (S3 UH, S4 entry) treat heterogeneity.
#
# Deliver como one table (tab_v3_s1_handoff.tex) with columns
# interpretable: size samplel, price-ref mean, bids by firm,
# HHI of suppliers, win-rate, spread of the bid vs ref.
#
# Also attach um diagnostic light: firms recurrent vs one-shot,
# auction-level count mean of bidders.
...
# arrow::read_parquet chokes on thrift stats of this parquet
# (high cardinality of the ym_enc). DuckDB reads without complaining.
con <- con_duck()
...
# Firm-level: recurrence within the stratum ---------------------------
...
cat("\n--- firm-level recurrence (HHI in points) ---\n", file = logf)
sink(logf, append = TRUE); print(firm_stats); sink()

# Merge and table single ----------------------------------------------
tab <- merge(
...
for (i in seq_len(nrow(tab))) {
  ...
  tex <- c(tex, sprintf(
    ...
    formt(r$median_preco_ref, big.mark = ",", scientific = FALSE),
    formt(r$n_firms, big.mark = ","),
...
tex <- c(tex,
  ...
  "\\items Group 65, 18-month window around March 2018. Pharma is the",
...
# Alert flag: are there strata where SME share fell or did not grow?
shift <- tab[, .(
