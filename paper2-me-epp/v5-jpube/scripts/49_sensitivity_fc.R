# ----------------------------------------------------------------------
# Sensitivity of the BNE decomposition to the three F_c regimes:
#   (1) losers-only    (S4 primeira version)
#   (2) all-bidders    (S4 final — MC3 fix)
#   (3) Turnbull NPMLE (S5 — point ID via winner left-censoring)
#
# For each regime, roda o MC BNE (identical ao 45) e reporta S1/S2/S3,
# Δtotal, share intensive/entry by pharma. The goal is to quantify
# the bias MC3 rigorously: if Turnbull ≈ all-bidders, v3 final is
# defensible; if Turnbull diverges, the headline must be revised.
#
# Sampling: inverse-CDF via approxfun of the F_c Turnbull (grid parquet).
# For losers-only e all-bidders, sample empirical direta of the bids.
#
# Output: tab_v3_sensitivity_fc.tex
...
# 1. Ashowss losers-only e all-bidders of the bids UH-clean -------------
...
# 2. Samples via inverse-CDF of the F_c Turnbull -------------------------
...
turnbull_sampler <- function(F_df, n) {
  # F_df: date.table with c, F_c_turnbull (ordenado by c).
  # Samples via inverse-CDF: u ~ U(0,1) → c = F^{-1}(u).
  if (nrow(F_df) < 2) return(numeric())
  u <- runif(n)
  # approx inversa: de F_c for c.
...
# 4. BNE simulation (identical ao 45, parametrizada by sample dict) --
...
# Compute gap Turnbull vs all-bidders (quanto v3 final if move).
...
cat("\n--- gap in Δtotal: Turnbull vs all, losers vs all ---\n",
    file = logf)
sink(logf, append = TRUE)
print(gap)
sink()
...
tex <- c(tex,
  ...
  "\\items Three F\\_c sampling regimes applied to the same BNE engine",
  ...
  "English-reverif IPV). Turnbull is the rigorous benchmark; ``all''",
