# ----------------------------------------------------------------------
# Estimate F_c^k (k = SME / non-SME) in Pregão from drop-outs (losers'
# final bids) under the English-reverse IPV interpretation: each loser
# exits the auction when the clock price reaches their cost, so
# c_loser = final_bid_loser. This is the Hong-Shum (2003) / Athey-Haile
# (2002) point identification.
#
# Under asymmetric IPV, "exit at cost" is still weakly dominant for
# each bidder regardless of rival types, so F_c^SME can be estimated
# and F_c^nonSME from each type's drop-outs separately.
#
# Two ECDFs reported by stratum:
#   losers-only: point ID (bias is small, only missing the lowest
#                cost, which is the winner's).
#   all-bidders: adds the winner; gives a one-sided upper bound on
#                F_c (winner stops before reaching their own cost).
#
# Primitive-invariance test: KS of F_c^nonSME_Pre vs F_c^nonSME_Post
# by pharma — the paper's identifying condition.
...
log_step("36", sprintf("n = %s firm×auction (window 18m)",
                       formt(nrow(m), big.mark = ",")), logf)

# 1. ECDF by stratum ------------------------------------------------
# grid common for c_norm: 0.005 to 2.0 with step 0.005
...
# Expande each lista for long formt
...
# 2. Summary by stratum: mean, p50, p75, p90 -------------------------
...
cat("\n--- KS shift test (Pre vs Post) by pharma × type ---\n", file = logf)
...
tex <- c(tex,
  ...
  "\\items Normalized by items reference price; values near 1 mean the",
  "firm bid cloif to the ceiling. Under English-reverse IPV, each loser",
...
p <- ggplot(fig_df, aes(x = c, y = F_c, color = period)) +
  ...
  labs(x = "c / reference price", y = expression(F[c](c)),
       title = "Pregão drop-out cost distributions (point ID)",
       color = "") +
