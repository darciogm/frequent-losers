...
# (state secretariats, code 1), "indirect" (autarchies, foundations,
# state companies; codes 3-6), and "convenied entities" (code 80).
# Replicates the v1 baseline spec g65_pre + convite + lquantidade |
# item_alt + date_oc_numb and reports the coefficient by group.
...
dat <- dbGetQuery(con, sprintf("
  SELECT lpreco_final, lquantidade, convite, item_alt, date_oc_numb,
         pbu_type_mgmt_code,
         CASE WHEN codigogroup = 65 THEN 1 ELSE 0 END AS g65,
         CASE WHEN date_oc_numb < 698 THEN 1 ELSE 0 END AS Pre,
...
# v1 spec: g65_pre + convite + lquantidade | item_alt + date_oc_numb
...
tex <- c(
  ...
  sprintf("\\textbf{Pooled (all buyer types)} & %s & %s & %.4f & %.4f & %s \\\\",
          formt(nobs(fit_pool), big.mark = ","),
          formt(dat[, sum(g65_pre)], big.mark = ","),
...
tex <- c(tex,
  ...
  "\\items Specification: $\\log p_{\\mathit{final},\\,it} = \\beta \\cdot",
  "(\\mathit{g65}_i \\cdot \\mathit{Pre}_t) + \\delta \\cdot \\mathit{convite}_{it}",
  "+ \\theta \\cdot \\log q_{it} + \\gamma_i + \\gamma_t + \\varepsilon_{it}$,",
  "with items ($\\gamma_i$) and month ($\\gamma_t$) fixed effects, and",
  "standard errorrs clustered by item. Sample: 18-month symmetric",
