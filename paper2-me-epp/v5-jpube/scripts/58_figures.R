# ----------------------------------------------------------------------
# Figures for the main text and appendix.
# Generates five new figures aligned with the paper's three findings,
# and rebuilds the two pre-existing panels with a clean LaTeX title.
# Design refs: Pollock (2025) on academic storytelling (show-and-tell)
# and Berengueres et al. (2019) on data-viz first principles — one
# figure, one claim; grayscale-safe; self-contained captions; small
# multiples over overplots.
...
fig1 <- ggplot(dec_long, aes(x = scenario, y = price,
  ...
  scale_y_continuous(
    ...
    expand = c(0, 0),
    labels = scales::number_formt(accuracy = 0.01)) +
  labs(
    x = NULL,
...
apv_ann <- apv_long[scenario == "V2 (in the entry)",
                    .(pharma_lbl,
                      y = dp + 0.03,
...
fig2 <- ggplot(apv_long, aes(x = scenario, y = dp, fill = scenario)) +
  ...
  geom_segment(date = apv_ratio,
               aes(x = 1, xend = 2,
                   y = delta_V0, yend = delta_V2),
               inherit.aes = FALSE,
               linetype = "dashed", colour = "grey40",
               linewidth = 0.3) +
  geom_text(date = apv_ann, aes(x = x, y = y, label = lbl),
            inherit.aes = FALSE, size = 3.0) +
  facet_wrap(~ pharma_lbl, nrow = 1) +
  scale_fill_manual(values = c(
    "V0 (endogenous)" = "grey45",
    "V2 (in the entry)"   = "grey75"),
    guide = "none") +
  scale_y_continuous(
    limits = c(0, 0.62),
...
# Strict-invariance fallback derived from canonical tab_v3_strict_invariance:
# Delta_gov_strict = 0.29 (non-pharma), 0.47 (pharma); welfare(lambda=0.30)
# anchored at 30.0% (non-pharma) and 39.2% (pharma) of p_S1 (0.777, 0.655);
# implied DWL_alloc_strict = 0.146 (non-pharma), 0.116 (pharma).
...
forest_df[, ypos := paste0("λ = ", formtC(lambda, formt = "f", digits = 2))]

fig3 <- ggplot(forest_df,
               aes(x = mean, y = ypos,
                   colour = spec, shape = spec)) +
  geom_errorrbarh(aes(xmin = lo, xmax = hi),
  ...
  annotate("text", x = 11.8, y = 0.55,
           label = "Reduced-form benchmark (11.8%)",
...
fig5 <- ggplot(weight_df,
  ...
  geom_point(size = 3.0) +
  geom_text(aes(label = sprintf("%.2f", w_star)),
            vjust = -1.1, size = 2.8, colour = "black",
            show.legend = FALSE) +
  facet_wrap(~ pharma_lbl, nrow = 1) +
  scale_colour_manual(values = c(
    "Main (equilibrium selection)" = "grey15",
