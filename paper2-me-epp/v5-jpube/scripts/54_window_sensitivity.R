# ----------------------------------------------------------------------
# Window sensitivity of the BNE decomposition: 18m (baseline v3) vs 12m
# vs 6m. Windows in Stata-monthly date units cinputs in cutoff 698
# (March 2018):
#   win_18m = [680, 715]  (v3 baseline)
#   win_12m = [686, 709]
#   win_06m = [692, 703]
#
# Join bids_uh_cleaned with g65_keys for trazer date_oc_numb.
# Entry counts recomputed by window (countsgin mean of bidders
# by auction within the window). Regime all-bidders (v3 final).
...
# Bids with date_oc_numb via join in g65_keys. ------------------------
bids <- dbGetQuery(con, sprintf("
  ...
  LEFT JOIN read_parquet('%s') k
    ON b.numerodaoc = k.numerodaoc AND b.codigoitems = k.codigoitem
  WHERE b.mod = 'pregao'
    AND b.c_norm_clean > 0 AND b.c_norm_clean <= 3
    AND b.period IN ('Pre','Post')
    AND k.date_oc_numb IS NOT NULL
...
compute_entry <- function(dt) {
  # Firbut distinct (cod_forn) by auction, after sum by type,
  # after mean by stratum. Without cod_forn in the unique, todos os SMEs
  # de um auction colapsam in 1 row — bug that I already paguei 1×.
...
for (name in names(windows)) {
  w <- windows[[name]]
  sub <- bids[date_oc_numb >= w[1] & date_oc_numb <= w[2]]
...
cat("\n--- window sensitivity: 3 windows × 2 pharma ---\n", file = logf)
...
for (i in seq_len(nrow(res))) {
  ...
  tex <- c(tex, sprintf(
    ...
    r$pharma_lbl, r$window, formt(r$n_auctions, big.mark = ","),
...
tex <- c(tex,
  ...
  "\\items Windows are in Stata monthly-date units centered at cutoff",
