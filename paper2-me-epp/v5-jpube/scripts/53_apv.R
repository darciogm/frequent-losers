# ----------------------------------------------------------------------
# APV (Alternative Policy Variants) — compare four againstfactuais
# in the BNE engine to show the range of policy effects:
#
#   V0 = v3 main: 100% set-aside, endogenous Post SME pool.
#   V1 = partial: 50% auctions SME-only (Post pool) + 50% open (Pre pool).
#   V2 = entry-isolated: SME-only with N^SME = N^SME_Pre (without entry).
#   V3 = price preference 10%: open with SMEs recebendo bid × 0.9.
#
# Baseline comparison = S1 (open, Pre) observed.
# Report: price, Δ vs baseline, share vs V0.
...
simulate_one <- function(n_sme, n_nonsme, fc_sme, fc_nonsme,
  ...
  # Price preference: SME bids are evaluatedos with desconto para
  # determinar winner, but o governo paga o c_(2) *actual*
  # (Vickrey-equivalent: winner pays what second bidder would need
  # to match, in bid real). Quando there is preference, o "second bidder"
  # already está with effective different of the actual, so pago o threshold
  # that o winner still getsria: actual_second if winner SME paga o
  # actual-equivalent of the non-SME boundary.
  c_s_eff <- c_s * sme_discount
  all_eff    <- c(c_s_eff, c_n)
  all_actual <- c(c_s,     c_n)
  ord <- order(all_eff)
  # Vickrey: price paid = segunda effective, but "traduzida" pelo
  # winner's preference. If winner é SME, ele podia ter bidado até
  # c_(2) effective / sme_discount without perder → pay this valor
  # actual; if winner é non-SME, payment = c_(2) effective actual
  # (non-SME not has discount, so effective = actual).
...
run_apv <- function(ph) {
  ...
  # Reset seed before each block so V0 matches Table tab_v3_bne_decomp's
  # mean_S3 (canonical main spec), V1/V2/V3 are conditional on the same
  # seed-state, and S1 here matches mean_S1 in tab_v3_bne_decomp.
  set.seed(20260423)
...
# Relative share: each V in % of delta_V0.
...
tex <- c(
  ...
  "\\toprule",
  "Class & $\\bar p_{S_1}$ & V0 (v3) & V1 (50\\%) & V2 (in the entry) & V3 (10\\% pref.) \\\\",
  "\\midrule")
...
tex <- c(tex,
  ...
  "\\items Price $\\Delta$ relative to observed $S_1$ (open, Pre pool).",
