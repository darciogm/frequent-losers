# ----------------------------------------------------------------------
# Entry rates by stratum: how many SMEs and non-SMEs bid in each
# auction, over the potential pool (firms that already bid in that
# CADMAT class in any moment).
#
# Step 1 of Athey-Levin-Seira (2011): identify the entry margin
# extensive. The ratio (bidders observed / potential pool) is the
# entry probability.
#
# DiD on entry rate Pre → Post, stratified by pharma:
#   Δ^SME, Δ^non-SME = effect of the policy on the propensity to enter
#
# Outputs:
...
# 1. Potential pool by CADMAT class × type -------------------------
# Firm is part of the pool if already bid in that class in any
# period (window Pre, Post, or fora). Use bid_level_sme_pharma_g65
# for construir a firm × class x type table.
...
pool <- dbGetQuery(con, sprintf("
  ...
  FROM read_parquet('%s')
  WHERE mod IN ('convite','pregao') AND codigoclass IS NOT NULL
  GROUP BY codigoclasse, sme_bec
", sme_path)) |> setDT()

cat("\n--- pool size by (class × type) (top 15) ---\n", file = logf)
sink(logf, append = TRUE)
print(pool[order(-pool_size)][1:15])
sink()

# Sum by pharma flag.
pool[, pharma_narrow := as.integer(codigoclass %in% cadmat_pharma_narrow)]
pool_pharma <- pool[, .(pool_size = sum(pool_size)),
                    by = .(pharma_narrow, sme_bec)]
cat("\n--- pool size by (pharma × type) ---\n", file = logf)
sink(logf, append = TRUE); print(pool_pharma); sink()

# 2. Entry count by auction and type -----------------------------------
# Count distinct firms by type in each auction×item. Without
# distinct, Pregão counts iterative bids and inflates 3-4×.
...
log_step("44", sprintf("auctions × period = %s",
                       formt(nrow(entry), big.mark = ",")), logf)
...
tex <- c(tex,
  ...
  "\\items Entry rates derived as bidders-observed / potential-pool",
