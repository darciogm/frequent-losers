...
# concentration (Gini, top-10%, top-1%), firm size (size_max from
...
# Group 65 SME winners post-policy (March 2018 = date_oc_numb 698+).
...
win <- dbGetQuery(con, sprintf("
  SELECT cnpj_fornecedor,
         ...
         municipio_forn,
         dist1,
         CASE WHEN date_oc_numb >= 698 THEN 'Post' ELSE 'Pre' END AS period
  FROM read_parquet('%s')
  WHERE codigogroup = 65
    AND oc_item_status = 1
    AND me_epp = 1
    AND valor_total_final > 0
    AND cnpj_fornecedor IS NOT NULL
    AND date_oc_numb BETWEEN 680 AND 715
", bec_path)) |> setDT()
...
rais <- dbGetQuery(con, sprintf("
  SELECT cnpj_raiz, size_max, vinc_ativos_sum, uf_forn_modal
  FROM read_parquet('%s')
", rais_path)) |> setDT()
...
# Distribution by firm size band (RAIS size_max)
# RAIS size_max codes (per Brazilian classification):
...
tex <- c(tex,
  ...
  "\\items \\textit{Notes:} Panel A reports the concentration of realized post-policy BEC Group-65 revenue across SME suppliers. The Gini and top-share statistics measure inequality in the realized transfer. Panel B reports the share of total Post revenue accruing to SME winners in each RAIS firm-size band, computed on the subset matched to RAIS 2017 (Brazilian matched employer-employee date). The ``Not in RAIS'' band collects firms not matched to the registry, typically the smallest or newest. Panel C reports the share of total Post revenue accruing to SME winners by geography. The set-aside's distributive footprint is heavily concentrated and concentrated specifically among micro firms in SP-interior municipalities; the typical beneficiary is small in employment terms but local in geography.",
  "\\end{tablenotes}",
  "\\end{threeparttable}",
  "\\end{table}")
