# ----------------------------------------------------------------------
# Robustez of the GPV inversion in Convite ao kernel bandwidth. O 42 roda
# with Silverman default (h_factor=1); here varremos h_factor ∈
# {0.5, 0.75, 1.0, 1.5, 2.0} to confirm c_0.5 and c_0.75 do not
# dependin materialmente of the bandwidth escolhida.
#
...
cat("\n--- quantiles c_0.5 e c_0.75 by bandwidth h_factor ---\n",
    file = logf)
sink(logf, append = TRUE)
print(res[, .(pharma_lbl, sme_lbl, period, h_factor, n_obs, c50, c75)])
sink()

# Range: max − min in c50 e c75 by stratum (qualitative robustness).
range_tab <- res[, .(
  n_h = .N,
...
tex <- c(tex,
  ...
  "\\items Each column reports $c_{0.5}$ of the GPV-inverted $F_c$ in",
