...
The pair of purchases introduced in Section~\ref{sec:data}---fifty thousand units of furosemida 40\,mg, CADMAT class 6531, one month before and eight months after the cutoff---makes the magnitudes tangible. Applying the pharmaceutical-class point estimates to that single purchase: relative to what the unit would have cost under the pre-period open regime, the government pays $\Delta_{\text{gov}} = +0.328$ of the reference price more under the set-aside. That fiscal incidence decomposes into $\mathrm{DWL}_{\text{alloc}} = 0.210$ in allocative waste, an implicit transfer of $0.118$ to the SME winner ($\Delta_{\text{gov}} - \mathrm{DWL}_{\text{alloc}}$), and an additional $\lambda \cdot \Delta_{\text{gov}} = 0.098$ in tax-distortion welfare cost at $\lambda = 0.30$. Total welfare loss is $0.210 + 0.098 = 0.308$, or 47.0 percent of $p_{S_1}$ (Table~\ref{tab:v3_welfare}, pharma row $\lambda = 0.30$). The per-auction number scales; the group-level fiscal consequence is substantial, and the welfare consequence is larger still.
