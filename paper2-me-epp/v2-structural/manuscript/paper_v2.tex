...
[0, 1]$. Table~\ref{tab:trim-sens} reports the decomposition under
three trimming schemes.

\begin{table}[htbp]
\centering
\caption{Decomposition sensitivity to boundary trimming}
\label{tab:trim-sens}
\small
\begin{tabular}{lrrr}
\toprule
Trim range $\hat c / P_{\text{ref}}$ & Retained sample & Intensive \% & Entry \% \\
\midrule
$(0.001, 1.5)$ (headline)  & 77.7\% & 66\% & 34\% \\
$(-0.5, 2.0)$ (permissive) & 94.3\% & 68\% & 32\% \\
$(0.01, 1.2)$ (strict)     & 63.4\% & 64\% & 36\% \\
\bottomrule
\end{tabular}
\end{table}

Across trim schemes, the 66/34 decomposition moves by at most
two percentage points. The primitive-invariance test result
(non-SME $D = 0.03$) is unchanged up to the third decimal under
any trim in $[-0.5, 2.0]$.
...
Bootstrap SE on the weighted-mean intensive share
(66\%) is 2.3 percentage points, yielding a 95\% CI of [61\%,
71\%]---the ordering
``intensive $>$ entry'' is preserved in 99\% of replicates.
Bootstrap SE on the DWL magnitude (R\$295k) is R\$18k,
yielding a 95\% CI of [R\$260k, R\$330k]. The primitive-invariance
test statistic (KS $D = 0.03$ for non-SME) has bootstrap SE of
0.007, leaving the economic-invariance conclusion robust.
...
\paragraph{By-$N$-bin shifts.}
Table~\ref{tab:ht-byN} reports KS shifts across the March 2018
break, by auction size, for the type-separated Preg\~ao G65
sample. The HT upper-bound shift is small and marginally
significant for SMEs ($D \leq 0.03$ at all $N$-bins) and large
and highly significant for non-SMEs ($D$ grows from 0.03 at
$N=2$ to 0.11 at $N \geq 5$, consistent with selection among
non-SMEs who continue bidding despite near-zero win probability
post-policy).

\begin{table}[htbp]
\centering
\caption{Preg\~ao Haile-Tamer bounds: shifts by $N$-bin}
\label{tab:ht-byN}
\small
\begin{tabular}{lrrr}
\toprule
$N$-bin & Non-SME shift (\% of ref) & Non-SME KS $D$ & SME KS $D$ \\
\midrule
$N = 2$   & $+2.2\%$ &  0.033 & 0.025 \\
$N = 3$   & $+5.8\%$ &  0.062 & 0.026 \\
$N = 4$   & $+9.6\%$ &  0.099 & 0.028 \\
$N \geq 5$& $+10.3\%$ & 0.108 & 0.027 \\
\bottomrule
\end{tabular}
\end{table}

The rising non-SME shift with $N$ is consistent with a
selection story: in larger auctions where the non-SME pool was
always thinner relative to the SME pool, the selection bias on
the remaining post-policy non-SME sub-pool is amplified.
...
\begin{tabular}{lrrr}
\toprule
Regressor & Coefficient & SE & $t$ \\
\midrule
$\log N$                        & $+0.0094$  & 0.0062 & $1.52$ \\
$\log P_{\text{ref}}$           & $+0.0015$  & 0.0014 & $1.07$ \\
SME indicator                    & $+0.0042$  & 0.0053 & $0.79$ \\
Pharma (class 6531) dummy       & $-0.0069$  & 0.0046 & $-1.50$ \\
\midrule
Joint Wald $F(2, 8{,}250)$ on $(\log N, \log P_{\text{ref}})$: &
 \multicolumn{3}{r}{$F = 1.50, p = 0.22$} \\
\bottomrule
\end{tabular}
\end{table}

\emph{Dependent variable}: $c_{\text{norm}}$ (normalized pseudo-cost).
\emph{Clustering}: standard errors clustered at the auction level.
\emph{Sample}: 18,907 post-filter bidder-auctions.
...
$-0.022$ (non-SME) and $-0.026$ (SME) of reference price---a
near-symmetric 4--5\% reduction across types. Because the
adjustment is symmetric, the decomposition weights and the DWL
magnitudes move by at most 2--3 percentage points relative to
the risk-neutral baseline:
\begin{itemize}
\item Fixed-pool decomposition: 66/34 (risk-neutral) $\to$ 68/32
(CARA-adjusted). The intensive margin's dominance is preserved.
\item DWL as \% of procurement: 11.8\% (risk-neutral) $\to$
11.2\% (CARA-adjusted). The welfare cost is robust.
\item Primitive-invariance test: non-SME KS $D$ moves from 0.030
to 0.031. Invariance conclusion unchanged.
\end{itemize}

The headline story does not hinge on assuming risk neutrality;
moderate CARA risk aversion tightens the cost recovery slightly
but preserves every qualitative conclusion.
