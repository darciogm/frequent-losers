# S3 / task 40 -------------------------------------------------------
# Decomposição de variância dos log-lances em componente auction-level
# (UH, α_t) e bidder-level (ε_it) — passo 0 de Krasnokutskaya (2011).
# Mostra quanto da variância está ao nível do leilão e justifica (ou
# não) a deconvolução.
#
# Modelo: y_it = log(b_it / ref_it) = a_t + e_it, com Var(a) = σ²_a,
# Var(e) = σ²_e. Por estrato (period × pharma × SME × modalidade).
#
# Estimador: método dos momentos (ANOVA simples). Robusto a N
# variável por leilão via média ponderada.
#
# Saída: output/tables/tab_v3_uh_variance.tex, logs/40_uh_variance.log
...
# 1. Monta base de log-bids por modalidade ---------------------------
# Convite: cada firm×auction tem 1 lance → c_norm = bid/ref. Pregão:
# drop-out = final_bid/ref. Uso o mesmo alvo (c_norm) em ambos.
...
# Convite: recria o b_norm com os mesmos filtros que script 38.
...
log_step("40", sprintf("base de bids = %s rows",
                       format(nrow(bids_all), big.mark=",")), logf)

# 2. Variance decomposition por estrato -----------------------------
...
cat("\n--- variance decomposition por estrato ---\n", file = logf)
...
# 3. Agregação: ρ por pharma × SME (média entre pre/post e modalidade)
rho_summary <- res[, .(
...
cat("\n--- rho (fração de variância no nível do leilão) ---\n",
    file = logf)
sink(logf, append = TRUE); print(rho_summary); sink()

# Salva parquet para S3.2 usar.
