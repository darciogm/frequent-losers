...
# Load completed-item DiD sample with channels already in BEC.
dat <- dbGetQuery(con, sprintf("
  SELECT b.lpreco_final, b.lquantidade, b.convite, b.item_alt, b.data_oc_numb,
  ...
  WHERE b.oc_item_status = 1
    AND b.data_oc_numb BETWEEN 680 AND 715
...
rais <- dbGetQuery(con, sprintf("
  SELECT cnpj_raiz, tamanho_max, cnae20_modal
  FROM read_parquet('%s')
", rais_path)) |> setDT()
dat <- rais[dat, on = "cnpj_raiz"]
dat[, size_band := fcase(
  is.na(tamanho_max) | tamanho_max <= 2, "0-4",
  tamanho_max <= 4, "5-19",
  tamanho_max <= 5, "20-49",
  tamanho_max <= 6, "50-99",
  default = "100+")]
...
# we compute β_full (with all channels) and δ_k × γ_k for each k.
gelbach_run <- function(channels, label) {
  controls_str <- paste(channels, collapse = " + ")
  fml_long <- as.formula(sprintf("lpreco_final ~ g65_pre + convite + lquantidade + %s | item_alt + data_oc_numb",
                                  controls_str))
  fit_long <- feols(fml_long, data = dat, cluster = ~item_alt)
...
cat(sprintf("β_short (no channel controls) = %.4f (SE %.4f)\n",
