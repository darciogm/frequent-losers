# S5 / task 51 -------------------------------------------------------
# Bootstrap CI (B=500) da BNE decomposition nos 3 regimes de F_c.
# Cluster ao nível de leilão (não bid individual) para respeitar a
# correlação within-auction causada por UH.
#
# Para cada bootstrap b:
#   1. Resample auctions com reposição (por estrato pharma×period×sme).
#   2. Recompute F_c em cada regime (losers/all/Turnbull).
#   3. Rodar BNE MC (B_MC=500) com cada regime e calcular
#      Δtotal, share_intens, share_entry.
#   4. Salvar 12 estatísticas (3 regimes × 2 pharma × 2 shares +
#      Δtotal).
#
# Final: 95% CI via percentis 2.5/97.5 sobre os B replicates.
# Paralelização via parallel::mclapply (12 workers).
#
# Output: tab_v3_bootstrap_ci.tex
...
# 3. Uma rodada de bootstrap -----------------------------------------
one_bootstrap <- function(bs_idx) {
  set.seed(seed_for_iter(51, bs_idx))
  # Resample auctions dentro de cada (period × pharma) com reposição.
  # sme_bec é bid-level; mantemos original dentro dos bids resampled.
  bs_bids <- auctions[, .(
  ...
  # Monta samples por estrato, 3 regimes.
...
log_step("51", sprintf("rodando B=%d em %d cores", B, n_cores), logf)
t0 <- Sys.time()
