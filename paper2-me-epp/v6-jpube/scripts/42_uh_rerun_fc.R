...
#   Convite: GPV inversion em c_norm_clean (logo após remover a_t).
#   Pregão : drop-out point ID em c_norm_clean dos perdedores.
#
# Compara F_c_clean vs F_c_raw e reporta:
#   (i) shift da median cost,
#   (ii) shift entre modalidades (Convite × Pregão) — UH correction
#        deve FECHAR o gap de 0.18-0.23 em non-pharma observado em S2.
#
# Saídas: convite_fc_uh.parquet, pregao_fc_uh.parquet,
...
# 1. Pregão drop-out F_c_clean (point ID em bids UH-cleaned) ---------
...
# 2. Convite GPV em bids UH-cleaned ---------------------------------
...
# 3. Tabela: mediana e gap Convite × Pregão antes/depois UH ---------
...
cat("\n--- c_50 antes/depois UH, por modalidade × estrato ---\n",
    file = logf)
...
# Gap entre modalidades (Convite vs Pregão) na mediana, antes vs depois UH
...
cat("\n--- cross-modality gap (mediana Pregão − Convite) antes/depois UH ---\n",
    file = logf)
sink(logf, append = TRUE)
