# S6 / task 55 ------------------------------------------------------
# Welfare decomposition da set-aside SME-only. Dois componentes de
# loss welfare:
#
#   DWL_alloc = E[c_(1)^{S3} - c_(1)^{S1}]
#      custo de produção adicional porque winner SME pode não ser
#      o bidder com menor custo overall.
#
#   MCPF_dist = Δ_gov · λ
#      distorção de taxation: cada R$ extra que gov paga custa
#      (1+λ) em welfare terms (λ = marginal cost of public funds).
#
#   Total welfare loss = DWL_alloc + MCPF_dist.
#   Reportado como % de p_S1 (baseline procurement value).
#
# Sensitivity em λ ∈ {0.20, 0.30, 0.40}. Regime all-bidders F_c (v3).
...
# Simula B auctions e retorna vetores: c1_S1, c2_S1, c1_S3, c2_S3
