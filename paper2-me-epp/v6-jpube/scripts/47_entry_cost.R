# S4 / task 47 -------------------------------------------------------
# Calibração do entry cost κ^k via zero-profit condition no margin.
# Para cada tipo, expected profit por entry:
#   κ^k = P(win|k) · E[π | k wins]
# onde π = c_(2) - c_(1) (Vickrey-equivalente; revenue equivalence
# em IPV). κ é estimado por MC: sorteia cost draws sob o entry
# profile observado, calcula o profit do vencedor, tipa-atribui
# (winner SME ganha π; non-SME winner → SME contribui 0). κ é a
# média não-condicional dos profits sob os sorteios; por construção,
# isto equivale a P(win) × E[π|win] sem double-counting.
#
# Saída: tab_v3_entry_cost.tex com κ^SME e κ^non-SME em R$ (ref-price
# scale) por pharma.
...
# Amostra de F_c: all-bidders (winner + loser), não só losers. Em
# Pregão BEC o bid final do winner ≈ c_(2) real, ou seja overstates
# c_win mas não por muito; losers-only, ao contrário, remove a cauda
# esquerda inteira. All-bidders é a correção direcional (Hong-Shum
# 2003 combined; Turnbull NPMLE fica em S5).
...
# Mediana do ref_price por pharma para converter ref-units → R$. Uso
# bids_uh_cleaned porque já carrega ref_price e pharma_narrow, e
# g65_keys não tem pharma_narrow. Mediana em vez de média: cauda do
# ref_price é pesada (non-pharma: mean 4.8k vs median 14).
...
set.seed(seed_for_script(47))

sim_profit <- function(fc_sme, fc_ns, n_sme, n_ns, B = 5000) {
  # NA em auctions inválidas (n<2); 0 em auctions onde o outro tipo
  # venceu. Isso deixa mean(profit_sme, na.rm=TRUE) = κ marginal por
  # construção (zero-contribution dos auctions que SME perdeu) sem
  ...
  for (b in seq_len(B)) {
    ...
    # Zero para o tipo que perdeu, prof para o que venceu. Assim
    # mean(profit_*, na.rm=TRUE) = P(win) × E[π|win] por construção.
    if (winner_type == "SME") {
      profit_sme[b] <- prof
      profit_ns[b]  <- 0
    } else {
...
for (ph in c(0, 1)) {
  ...
  sim <- sim_profit(fc_sme, fc_ns, n_sme, n_ns)
  # P(win) = fração de auctions válidas em que o tipo venceu.
  p_win_sme <- mean(sim$sme > 0, na.rm = TRUE)
  p_win_ns  <- mean(sim$ns  > 0, na.rm = TRUE)
  # E[π | win] = média condicional a vencer (inclui só auctions
  # efetivamente vencidas pelo tipo).
  E_pi_sme <- mean(sim$sme[sim$sme > 0], na.rm = TRUE)
  E_pi_ns  <- mean(sim$ns[ sim$ns  > 0], na.rm = TRUE)
  # κ marginal = P(win) × E[π | win]. Equivalente à média não-
  # condicional (construção acima garante os zeros no tipo que perdeu).
