...
#   impose F_c^{SME,Post} = F_c^{SME,Pre} in the BNE decomposition
#   and welfare arithmetic, while keeping N^SME_Post as the entry
#   response. This is the limiting-case reading of Assumption a:setaside.
#
# The LaTeX table reports the benchmark figures already computed by the
# author for the E-refinado robustness appendix.
...
raw_welfare <- rbindlist(lapply(c(0, 1), function(ph) {
  ...
  sim <- simulate_welfare(n_pre$n_sme, n_pre$n_nonsme, n_post$n_sme,
