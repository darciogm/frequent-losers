...
The item-level reinterpretation of the R\$\,\setAsideThreshBRL{} threshold
creates a natural incentive for buyers to split or restructure purchase
notices in order to push items below it. A McCrary density test at
$\log\setAsideThreshBRL$ fails to detect such manipulation in either period
(post: T = \bunchingDensityTPost{}, p = \bunchingDensityPPost{}; pre:
T = \bunchingDensityTPre{}, p = \bunchingDensityPPre{}). Group~65 items also
sit well below the threshold in mass---over 90 percent of items have
reference values below R\$\,30,000---so any manipulation around the
R\$\,\setAsideThreshBRL{} threshold would be visible if present. The negative
result is consistent with the reference-price evidence in
