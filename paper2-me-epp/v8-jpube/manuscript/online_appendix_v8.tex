...
The reduced-form evidence is used to validate timing, sign, and approximate
scale. It is not the source of the structural magnitude. The diagnostics
below document pre-treatment imbalance that prevents a reduced-form-only
design, placebo cutoffs that are smaller than the real cutoff but not all
zero, and modern DiD estimators that attenuate the point estimate relative
to TWFE. Taken together, they support the role assigned in the main paper:
external validation of the institutional episode, not identification of the
structural price magnitude.
...
The recovery diagnostics do not prove IPV. They ask whether the
exclusion-dominant decomposition is mechanically generated by winner
censoring, common auction-level scale heterogeneity, or cross-format
inconsistency in willingness-to-supply recovery. The checks below report
alternative treatments of the winner observation, a multiplicative
auction-level heterogeneity correction, and a cross-modality comparison
between Preg\~ao drop-outs and Convite GPV recovery, together with KS
distances that locate where the recovery is most credible and where
policy rankings must be treated as conditional.
...
dependence, coordination, and reference-price evolution. They do not show
that composition is irrelevant, that conduct is competitive, or that
reference-price changes are absent. They show that the price-formation
ranking is not mechanically generated by any single one of those channels;
welfare-ranking sensitivity is the more conditional object and is the
locus of the pharmaceutical boundary case.
