...
\section{Cost Recovery and Structural Estimation}
...
\centering
\begin{threeparttable}
\caption{Winner-censoring and cost-recovery checks}
...
\centering
\begin{threeparttable}
\caption{Cost-distribution quantiles under alternative winner treatments}
...
\begin{tabular}{lcccc}
\toprule
Specification & \multicolumn{2}{c}{Net price effect} & \multicolumn{2}{c}{Exclusion share} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
 & Non-pharma & Pharma & Non-pharma & Pharma \\
\midrule
Baseline & \bneEffectTotalNp{} & \bneEffectTotalPh{} & \bneShareIntensNp\% & \bneShareIntensPh\% \\
Strict invariance & \siDeltaTotalNp{} & \siDeltaTotalPh{} & \strictShareNp\% & \strictSharePh\% \\
Turnbull cost recovery & \costRegimeTurnNp{} & \costRegimeTurnPh{} & \turnbullShareNp\% & \turnbullSharePh\% \\
18-month window & \windowTotalEighteenNp{} & \windowTotalEighteenPh{} & \windowShareEighteenNp\% & \windowShareEighteenPh\% \\
12-month window & \windowTotalTwelveNp{} & \windowTotalTwelvePh{} & \windowShareTwelveNp\% & \windowShareTwelvePh\% \\
6-month window & \windowTotalSixNp{} & \windowTotalSixPh{} & \windowShareSixNp\% & \windowShareSixPh\% \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
\subsection{Reference-price validation}

All structural prices are normalized by buyer reference prices, so mechanical
changes in reference-price setting would be a problem. I therefore work with
normalized cost and price objects and use the reduced-form price/reference
benchmark as an external check. The structural net effects,
\structRatioNp{} percent of the open-regime price in non-pharmaceuticals and
\structRatioPh{} percent in pharmaceuticals, are larger than the DiD-implied
benchmark of \didImpliedRatio{} percent. That gap is consistent with the fact
that the structural exercise holds the relevant bidder pools fixed and
simulates the order statistic directly, while the reduced form averages over
take-up, controls, and product-time variation. The gap is a reason to interpret
levels carefully, not a reason to discard the mechanism.
