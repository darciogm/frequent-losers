...
The second is an ensemble classifier that assigns each POI to one
of three regimes: ordinary, administrative-request, or litigated.
...
across four signal channels: a TF-IDF + LinearSVC model trained on
$\sim$170{,}000 ground-truth-labeled tender notices, regex matching
on tender-notice free text, regex matching on the JSON \textit{municipio}
metadata field, and positional heuristics on the notice header and
``local de entrega'' block. Signals are combined by weighted vote,
with the ML and metadata channels each sufficient individually and
regex-plus-positional requiring corroboration. We validate the
ensemble on a stratified hand-labeled sample of \BPregexNotices{}
notices ($\sim$\BPregexPerClass{} per predicted class); detailed
metrics and the attenuation analysis are in the Online
Appendix.\footnote{The classifier exploits textual markers whose
presence is independent of the price outcomes we estimate, so any
residual misclassification attenuates rather than biases our
coefficients.}
