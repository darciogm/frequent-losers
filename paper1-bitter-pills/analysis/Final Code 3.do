*** Decomposing effects (5th choice)
...
eststo clear
xtset item_id
eststo: quietly xtreg bid_qty_log $controls4 if po_firm_winner==1 & jud_adm==0 & po_proc_code==3, fe
esttab using quantity_fitted_for_jud_1_3.rtf, b(%9.4f) se(%9.4f) ar2  title(Fitting Quantities for Litigated Bids) indicate("PBU = dpbu_code2" "Time Trend = dm_y2") compress  replace
...
eststo: quietly xtreg bid_price_ref_log $qty2 $controls4 if po_firm_winner==1 & po_proc_code==3 & jud_adm==0, fe
...
eststo: quietly xtreg bid_price_ref_log $qty3 $controls4 if po_firm_winner==1 & po_proc_code==3 & jud_adm==0, fe
...
eststo: quietly xtreg bid_price_ref_log $qty4 $controls4 if po_firm_winner==1  & po_proc_code==3 & jud_adm==0, fe
...
eststo: quietly xtreg bid_price_ref_log $qty5 $controls4 if po_firm_winner==1  & po_proc_code==3 & jud_adm==0, fe
...
eststo: quietly xtreg bid_price_ref_log $qty6 $controls4 if po_firm_winner==1  & po_proc_code==3 & jud_adm==0, fe
...
esttab using price_ALT_FITTED_3.rtf, b(%9.4f) se(%9.4f) ar2 keep($qty6) title(Fitting Prices for Litigated Bids) indicate("PBU = dpbu_code2" "Time Trend = dm_y2") compress  replace
...
eststo: quietly xtreg fit_price_ref2 jud_adm if po_firm_winner==1 & po_proc_code==3, fe
eststo: quietly xtreg fit_price_ref3 jud_adm if po_firm_winner==1 & po_proc_code==3, fe
eststo: quietly xtreg fit_price_ref4 jud_adm if po_firm_winner==1 & po_proc_code==3, fe
eststo: quietly xtreg fit_price_ref5 jud_adm if po_firm_winner==1 & po_proc_code==3, fe
eststo: quietly xtreg fit_price_ref6 jud_adm if po_firm_winner==1 & po_proc_code==3, fe
...
esttab using watchdog_effect_FINAL_3.rtf, b(%9.4f) se(%9.4f) keep(jud_adm) title(Strict Liability Effect (Dep. Variable: Log Fitted Reserve Price)) mtitles("Order 2" "Order 3" "Order 4" "Order 5" "Order 6")  compress  replace
