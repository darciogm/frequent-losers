*** Decomposing effects (6th choice)
...
eststo: quietly xtreg bid_price_ref_log $qty2 $controls4 if po_firm_winner==1 & po_proc_code==3
...
eststo: quietly xtreg bid_price_ref_log $qty3 $controls4 if po_firm_winner==1 & po_proc_code==3
...
eststo: quietly xtreg bid_price_ref_log $qty4 $controls4 if po_firm_winner==1  & po_proc_code==3
...
eststo: quietly xtreg bid_price_ref_log $qty5 $controls4 if po_firm_winner==1  & po_proc_code==3
...
eststo: quietly xtreg bid_price_ref_log $qty6 $controls4 if po_firm_winner==1  & po_proc_code==3
...
*eststo: quietly xtreg fit_price_ref jud_adm dm_y* dpbu_code* if po_firm_winner==1 & po_proc_code==3
...
esttab using watchdog_effect_FINAL.rtf, b(%9.4f) se(%9.4f) keep(jud_adm) title(Strict Liability Effect (Dep. Variable: Log Fitted Reserve Price)) mtitles("Order 1" "Order 2" "Order 3" "Order 4" "Order 5" "Order 6") indicate("PBU = dpbu_code2" "Time Trend = dm_y2") compress  replace
