...
!!! success "No broad same-firm markup in deep repeated urgent markets"
    Within firm-buyer-item triples observed under both urgent regimes (1,206 triples, 4,573 observations), the administrative coefficient is **β̂ = 0.035 (SE 0.041)** — statistically indistinguishable from zero. Conditional on the same firm, buyer, and item, prices are indistinguishable across urgent regimes in deep markets. This is a deep-market null, not a universal claim: a residual within-firm gap persists in the earliest cohorts, while the quantity dimension reflects scale rather than same-firm pricing.
