...
* Keep only items that have both
...
file write tex "    \small" _n
file write tex `"    \item \textit{Notes:} Sample restricted to preg\~ao auctions and items with at least one ordinary and one litigated purchase (`n_items' unique items). Continuous variables winsorized at the 1st and 99th percentiles. Standard deviations in parentheses."' _n
...
di ""
tabstat bid_price_ref_log bid_price_log bid_qty_log ln_n_firms ln_n_bids po_firm_winner, by(purchase_type) statistics(mean sd N) format(%12.3f) columns(statistics)
