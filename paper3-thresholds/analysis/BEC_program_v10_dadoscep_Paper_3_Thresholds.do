...
collapse (min) bid_unit_price_negot_min=bid_unit_price_negot bid_ref_price_min=bid_ref_price (max) bid_unit_price_negot_max=bid_unit_price_negot bid_ref_price_max=bid_ref_price, by(po_item_id)
...
*----------------------------------------------------------------------------------------------------------------------------------------------------------
log using BEC_pregao.log, replace      																// Open log file
...
*drop unique_firm
rename nvals unique_firm
save BEC_pregao.dta, replace



*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 4.2. Collapsing bid_price 
*----------------------------------------------------------------------------------------------------------------------------------------------------------
collapse (sum) n_firms=unique_firm n_bids=bid_count (min) min_bid_price=bid_unit_price (max) max_bid_price=bid_unit_price (mean) mean_bid_price=bid_unit_price (median) median_bid_price=bid_unit_price (sd) sd_bid_price=bid_unit_price (semean) semean_bid_price=bid_unit_price, by(po_item_id)


save BEC_pregao_collapse_1.dta, replace																		// Saving new dta
clear all
capture log close       																				// Close existing log files
*----------------------------------------------------------------------------------------------------------------------------------------------------------
*----------------------------------------------------------------------------------------------------------------------------------------------------------
...
*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 4.3. Appending BEC_pregao and Collapsed dta
*----------------------------------------------------------------------------------------------------------------------------------------------------------

* Appending Collapse_1: bid_unit_price --> info about po
log using BEC_pregao_collapse_1.log, replace      																// Open log file

use BEC_pregao_collapse_1.dta, clear 																			// Database used
sort po_item_id
save BEC_pregao_collapse_1.dta, replace

use BEC_pregao.dta, clear 																			// Database used
sort po_item_id

merge m:1 po_item_id using BEC_pregao_collapse_1.dta
drop bid_count unique_firm
drop _merge
save BEC_pregao_final_1.dta, replace																		// Saving new dta
clear all
capture log close       


* Appending Collapse_2: bid_winner --> po (Successful or not?)
log using BEC_pregao_collapse_2.log, replace      																// Open log file

use BEC_pregao_final_1.dta, clear 																			// Database used
sort po_item_id
collapse (sum) sum_po_winner=bid_winner (max) po_winner=bid_winner, by(po_item_id)
save BEC_pregao_collapse_2.dta, replace																		// Saving new dta
clear all
capture log close


use BEC_pregao_collapse_2.dta, clear 																			// Database used
sort po_item_id
save BEC_pregao_collapse_2.dta, replace

use BEC_pregao_final_1.dta, clear 																			// Database used
sort po_item_id

merge m:1 po_item_id using BEC_pregao_collapse_2.dta
drop _merge
save BEC_pregao_final_2.dta, replace																		// Saving new dta
clear all
capture log close    
...
capture log close
cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/FINAL"
log using BEC_pregao_collapse_3.log, replace      																// Open log file

use BEC_pregao_final_2.dta, clear 																			// Database used
sort po_item_id
generate double bid_time_date = clock(bid_time, "YMDhms")
format bid_time_date %tc
drop bid_time
collapse (min) bid_time_min=bid_time_date (max) bid_time_max=bid_time_date, by(po_item_id)
save BEC_pregao_collapse_3.dta, replace																		// Saving new dta
clear all
capture log close


use BEC_pregao_collapse_3.dta, clear 																			// Database used
sort po_item_id
save BEC_pregao_collapse_3.dta, replace

use BEC_pregao_final_2.dta, clear 																			// Database used
sort po_item_id


merge m:1 po_item_id using BEC_pregao_collapse_3.dta
drop _merge
gen proc_length_seconds = (bid_time_max - bid_time_min)/1000
gen proc_length_minutes = (bid_time_max - bid_time_min)/60000
gen proc_length_hours = (bid_time_max - bid_time_min)/3600000
...
save BEC_pregao_final_3.dta, replace																		// Saving new dta
clear all
capture log close    

*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 4.4. Geocoding PBU and firm addresses
*----------------------------------------------------------------------------------------------------------------------------------------------------------
...
*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 4.4.1. Geocoding using Base 1: https://www.base-dados-cep.com/
*----------------------------------------------------------------------------------------------------------------------------------------------------------
...
capture log close
cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/FINAL"
log using BEC_pregao_collapse_4.log, replace      																// Open log file
...
use BEC_pregao_final_3.dta, clear 																			// Database used
sort pbu_zipcode
...
save BEC_pregao_final_4.dta, replace
...
capture log close
cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/FINAL"
log using BEC_pregao_collapse_5.log, replace      																// Open log file
...
use BEC_pregao_final_4.dta, clear 																			// Database used
sort firm_zipcode
...
save BEC_pregao_final_5.dta, replace
...
*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 4.4.2. Geocoding using Base 2: http://www.qualocep.com/
*----------------------------------------------------------------------------------------------------------------------------------------------------------
...
capture log close
cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/FINAL"
log using BEC_pregao_collapse_6.log, replace      																// Open log file
...
use BEC_pregao_final_5.dta, clear 																			// Database used
sort pbu_zipcode
...
save BEC_pregao_final_6.dta, replace
...
capture log close
cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/FINAL"
log using BEC_pregao_collapse_7.log, replace      																// Open log file
...
use BEC_pregao_final_6.dta, clear 																			// Database used
sort firm_zipcode
...
save BEC_pregao_final_append.dta, replace
...
use BEC_pregao_final_append.dta, clear
collapse (min) bid_unit_price_negot_min=bid_unit_price_negot bid_ref_price_min=bid_ref_price (max) bid_unit_price_negot_max=bid_unit_price_negot bid_ref_price_max=bid_ref_price, by(po_item_id)



save BEC_pregao_collapsed_ref_price.dta, replace
sort po_item_id

use BEC_pregao_final_append.dta, clear 																			// Database used
sort po_item_id

merge m:1 po_item_id using BEC_pregao_collapsed_ref_price.dta
*ren _merge _merge_20
drop _merge
save BEC_pregao_final_append.dta, replace
...
drop bid_status bid_id bid_time_min bid_time_max address_pbu ddd_pbu address_firm ddd_firm latit_pbu longit_pbu latit_firm longit_firm
save Final_Database_Thesis_1.dta, replace



*PBU Geocoding

cd "/home/darciogm1/projetos/bitter-pills/data/raw/bec-procurement/Appending Files"
...
use Final_Database_Thesis_1.dta, clear 																			// Database used
sort pbu_zipcode
...
save Final_Database_Thesis_20.dta, replace
...
use Final_Database_Thesis_20.dta, clear 																			// Database used
sort firm_zipcode
...
save Final_Database_Thesis_30.dta, replace
...
save Final_Database_Thesis_40.dta, replace
...
*----------------------------------------------------------------------------------------------------------------------------------------------------------
* 6. Extracting subsample: threshold around july 2018 and values to compare (counterfactual)
*----------------------------------------------------------------------------------------------------------------------------------------------------------
version 13              																				// Set Version number for backward compatibility
set more off            																				// Disable partitioned output
clear all               																				// Start with a clean slate
