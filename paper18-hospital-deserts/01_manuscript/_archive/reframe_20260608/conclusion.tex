...
evidence of a large acute mortality increase. Among the populations with
realized patient-flow ties to the closing hospitals, suicide mortality changed by $\valSuicWAtt$ per
...
million SUS admissions and checked by a sharp post-closure fall in
travel burden, recovers a population a generic distance-based analysis
would often miss. The measurement correction is the paper's portable
...
closure studies can estimate effects on a population that differs from
the one that used the closing hospital.
...
claim that deinstitutionalization is safe everywhere or that subtle effects
are absent. What we establish is narrower and, for a contested
global policy, useful: when Brazil closed these psychiatric hospitals,
the data do not show a large acute mortality increase in the populations
