...
We report three results in sequence. First, a validation: the
patient-flow exposure rule captures populations that genuinely depended
on the closing hospital, evidenced by a sharp fall in travel burden
(\ref{sec:results-firststage}). Second, the headline: among those
exposed populations, the mortality the reform's critics feared did not
materialize (\ref{sec:results-mortality}). Third, a result we decline to
...
% §5.1 — First stage
% =====================================================================
\subsection{First stage: the flow rule captures real dependence}
\label{sec:results-firststage}

Before asking what happened to the exposed populations, we verify that
the flow exposure rule identifies municipalities that actually relied on
the closing hospital rather than ones merely near it. The test is
mechanical and sharp: if exposed patients had been traveling far for
care that the closing facility uniquely provided, then closure should
cut the distance they travel for inpatient care, because the long trips
the facility generated cease.
...
magnitude, because it dilutes the genuinely dependent municipalities
...
sending patients on long trips to the facility that closed: these are
the populations a closure plausibly affects, and the ones a distance
rule misclassifies.
...
\begin{figure}[h!]
    ...
    falls after closure, confirming that exposed patients had been
    traveling far for care with no local substitute.}
...
% =====================================================================
\subsection{The feared outcome: no mortality catastrophe}
...
The answer is no. Figure~\ref{fig:es-mortality} reports the event-study
...
trajectory hovers around zero with no upward drift of the kind a
mortality shock would produce.
...
estimate therefore \emph{rules out} the large mortality increases that
deinstitutionalization's critics predicted---a one-quarter rise in
suicide among the exposed would lie outside the interval---but it does
\emph{not} establish a precise zero: effects smaller than that bound the
data cannot resolve at $N = \valNclosuresPnash$ closures (formal
minimum detectable effects are in Appendix~\ref{app:power}). The policy reading is
correspondingly bounded. The catastrophe did not happen to the
populations that bore the closures; we cannot speak to subtle harms, and
do not.

\input{tables/tab_mortality_null.tex}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{../04_figures/fig_es_mortality.pdf}
    \caption{The feared outcome. Population-weighted staggered
...
Both weightings deliver a statistically null effect with confidence
...
Read naively, this looks like the access deterioration one would expect
