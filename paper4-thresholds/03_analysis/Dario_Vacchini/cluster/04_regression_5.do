...
capture file close myfile_5  // Close any existing file handle named myfile

*** 0. Import data
use "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/data/final/df_convite_winner_looser.dta", clear

drop if market_item == 2644

* Open a CSV file to store results
file open myfile_5 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_pot_cumprof_120_std_market.csv", write replace
file write myfile_5 "Variable, Coefficient, Std. Error, Market Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
    ...
    if `n' >= 30 {
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_5 "`var', `coef', `stderr', `market', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_pot_cumprof_120_std_market.xls" , excel tex(frag) bdec(4) label ctitle(pot_cumprof_120_std_market_`market') addtext(Year FE, YES)
    }
    else {
