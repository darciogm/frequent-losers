...
file open myfile_4 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_limited_firm_by_item_class.csv", write replace
file write myfile_4 "Variable, Coefficient, Std. Error, Class Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
...
foreach item_class in `item_classes' {
    * Increment the counter
    ...
    if `n' >= 20 {
        ...
        reghdfe limited_firm i.flagvencedor##c.MV if códigoclasse == `item_class' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_4 "`var', `coef', `stderr', `item_class', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_limited_firm_by_item_class.xls" , excel tex(frag) bdec(4) label ctitle(limited_firm_item_class_`item_class') addtext(Year FE, YES)
    }
    else {
