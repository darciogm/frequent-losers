...
file open myfile_1 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_won_t_minus_1_by_item_group.csv", write replace
file write myfile_1 "Variable, Coefficient, Std. Error, Group Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
...
foreach item_group in `item_groups' {
    * Increment the counter
    ...
    if `n' >= 20 {
        ...
        reghdfe won_t_minus_1_item_group i.flagvencedor##c.MV if códigogrupo == `item_group' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_1 "`var', `coef', `stderr', `item_group', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_won_t_minus_1_by_item_group_by_item_group.xls" , excel tex(frag) bdec(4) label ctitle(won_t_minus_1_item_group_`item_group') addtext(Year FE, YES)
    }
    else {
