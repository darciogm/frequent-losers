...
file open myfile_2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_item_class_std_by_item_class_2.csv", write replace
file write myfile_2 "Variable, Coefficient, Std. Error, Class Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
...
foreach item_class in `item_classes' {
  * Increment the counter
    ...
    * Skip markets before 2000
    if `count' < 1500 {
        continue
    }

    * Stop the loop after 3500 markets
    if `count' > 3500 {
        break
    }
    ...
    if `n' >= 20 {
        ...
        reghdfe cumprof_won_90_item_class_std i.flagvencedor##c.MV if códigoclasse == `item_class' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_2 "`var', `coef', `stderr', `item_class', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_item_class_std_2.xls" , excel tex(frag) bdec(4) label ctitle(cumprof_won_90_item_class_std_`item_class') addtext(Year FE, YES)
    }
    else {
