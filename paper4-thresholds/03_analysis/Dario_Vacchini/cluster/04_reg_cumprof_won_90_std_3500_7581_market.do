...
file open myfile_4 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_std_3500_7581_market.csv", write replace
...
foreach market in `market_items' {
    ...
    * Skip markets before 3500
    if `count' < 3500 {
        continue
    }

    * Stop the loop after 7500 markets
    if `count' > 7581 {
        break
    }
    ...
    if `n' >= 20 {
        ...
        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_std_3500_7581_market.xls" , excel tex(frag) bdec(4) label ctitle(cumprof_won_90_std_`market') addtext(Year FE, YES)
    }
    else {
