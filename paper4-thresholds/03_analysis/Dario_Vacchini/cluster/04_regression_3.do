...
file open myfile_3 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_last_bid_market.csv", write replace
...
foreach market in `market_items' {
    ...
    if `n' >= 30 {
        ...
        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_last_bid_market.xls" , excel tex(frag) bdec(4) label ctitle(reg_last_bid_market_`market') addtext(Year FE, YES)
    }
    else {
