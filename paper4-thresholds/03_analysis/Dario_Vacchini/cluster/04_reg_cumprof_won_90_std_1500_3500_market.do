...
file open myfile_3 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_std_1500_3500_market.csv", write replace
file write myfile_3 "Variable, Coefficient, Std. Error, Market Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
...
foreach market in `market_items' {
    ...
    * Stop the loop after 4000 markets
    ...
    if `n' >= 20 {
        ...
        reghdfe cumprof_won_90_market_item_std i.flagvencedor##c.MV if market_item == `market' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_3 "`var', `coef', `stderr', `market', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_std_1500_3500_market.xls" , excel tex(frag) bdec(4) label ctitle(cumprof_won_90_std_`market') addtext(Year FE, YES)
    }
    else {
