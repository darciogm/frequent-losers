...
centile cumprof_won_90_std, centile(0.001 99.999)

* Remove outliers below the 1st percentile and above the 99th percentile
keep if cumprof_won_90_std >= r(c_1) & cumprof_won_90_std <= r(c_2)

* RD plot of backlog 90 days
rdplot cumprof_won_90_std prop_delta_bid if inrange(prop_delta_bid, -0.5, 0.5) & cumsum_part>10, vce(nncluster auction_num) graph_options(legend(off) xtitle(Deviation in bids) ytitle(90-Day Standardized Backlog) graphregion(color(white)) ylabel(-1.5(0.5)2.5, angle(0)))

* Save Plot
graph export "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/graphs/rd_plot_backlog_90_days.pdf", replace
