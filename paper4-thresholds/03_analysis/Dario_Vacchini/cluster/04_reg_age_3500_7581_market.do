...
file open myfile_4 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_age_3500_7581_market.csv", write replace
...
foreach market in `market_items' {
    ...
    * Stop the loop after 7500 markets
    ...
    if `n' >= 30 {
        ...
        reghdfe age i.flagvencedor##c.MV if market_item == `market' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_age_3500_7581_market.xls" , excel tex(frag) bdec(4) label ctitle(last_bid_`market') addtext(Year FE, YES)
    }
    else {
