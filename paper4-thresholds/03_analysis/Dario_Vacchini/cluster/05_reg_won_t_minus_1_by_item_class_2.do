...
file open myfile_2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_won_t_minus_1_by_item_class_2.csv", write replace
...
foreach item_class in `item_classes' {
    ...
    * Stop the loop after 4000 markets
    if `count' > 433 {
        break
    }
    ...
    if `n' >= 20 {
        ...
        reghdfe won_t_minus_1_item_class i.flagvencedor##c.MV if códigoclasse == `item_class' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_won_t_minus_by_item_class_2.xls" , excel tex(frag) bdec(4) label ctitle(won_t_minus_1_item_class_`item_class') addtext(Year FE, YES)
    }
    else {
