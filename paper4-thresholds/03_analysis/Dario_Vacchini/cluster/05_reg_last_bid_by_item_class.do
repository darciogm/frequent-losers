...
file open myfile_2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_last_bid_by_item_class.csv", write replace
...
foreach item_class in `item_classes' {
    * Increment the counter
    ...
    if `n' >= 20 {
        ...
        reghdfe last i.flagvencedor##c.MV if códigoclasse == `item_class' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_last_bid_by_item_class.xls" , excel tex(frag) bdec(4) label ctitle(last_bid_item_class_`item_class') addtext(Year FE, YES)
    }
    else {
