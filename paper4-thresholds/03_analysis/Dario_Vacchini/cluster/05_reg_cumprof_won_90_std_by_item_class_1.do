...
file open myfile_1 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_cumprof_won_90_item_class_std_by_item_class_1.csv", write replace
file write myfile_1 "Variable, Coefficient, Std. Error, Class Item, Sig_1pct, Sig_5pct, Sig_10pct" _n
...
foreach item_class in `item_classes' {
    ...
    * Stop the loop after 750 markets
    if `count' > 750 {
        break
    }
    ...
    if `n' >= 20 {
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_1 "`var', `coef', `stderr', `item_class', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_share_won90_by_item_class_1.xls" , excel tex(frag) bdec(4) label ctitle(cumprof_won_90_item_class_std_`item_class') addtext(Year FE, YES)
    }
    else {
