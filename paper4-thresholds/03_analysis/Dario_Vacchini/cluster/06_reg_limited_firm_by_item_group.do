...
capture file close myfile_4  // Close any existing file handle named myfile
...
file open myfile_4 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_limited_firm_by_item_group.csv", write replace
file write myfile_4 "Variable, Coefficient, Std. Error, Group Item, Sig_1pct, Sig_5pct, Sig_10pct" _n

* RD regression for each market
levelsof códigogrupo, local(item_groups)

* Initialize a counter
local count = 0

foreach item_group in `item_groups' {
    * Increment the counter
    local count = `count' + 1

    // Count the number of observations for each market_item
    count if códigogrupo == `item_group' & MV < 0.01 & MV > -0.01
    local n = r(N) // Store the count in a local macro

    // Run the regression only if there are more than 10 observations
    if `n' >= 20 {
        di "Running regression for item_group = `item_group' with `n' observations"
        
        reghdfe limited_firm i.flagvencedor##c.MV if códigogrupo == `item_group' & MV < 0.01 & MV > -0.01, abs(year)
        ...
        foreach var of local varnames {
            if strpos("`var'", "1.flagvencedor") {  // Check if the variable name contains "1.flagvencedor"
                ...
                file write myfile_4 "`var', `coef', `stderr', `item_group', `sig_1pct', `sig_5pct', `sig_10pct'" _n
            }
        }

        * Optionally, save regression results in Excel as before
        outreg2 using "/cluster/work/lawecon/Projects/procurement_brazil/dario_vacchini/output/tables/reg_limited_firm_by_item_group.xls" , excel tex(frag) bdec(4) label ctitle(limited_firm_item_group_`item_group') addtext(Year FE, YES)
    }
    else {
        di "Skipping códigoclasse = `item_group' due to insufficient observations (`n')"
    }
}
